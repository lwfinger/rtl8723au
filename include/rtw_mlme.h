...
struct mlme_priv {
	...
	u32 scan_start_time; /* used for time spent in scanning */
...
int rtw_init_mlme_priv(struct rtw_adapter *adapter);
...
void rtw_update_ht_cap(struct rtw_adapter *padapter,
		       u8 *pie, uint ie_len);
...
void rtw_roaming(struct rtw_adapter *adapter,
		 struct wlan_network *tgt_network);
