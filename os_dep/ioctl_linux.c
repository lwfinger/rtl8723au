				...
				char *start, char *stop)
{
...
#ifdef CONFIG_WAPI_SUPPORT
	{
		...
		DBG_8723A("rtw_wx_get_scan: %s ",pnetwork->network.Ssid.Ssid);
		DBG_8723A("rtw_wx_get_scan: ssid = %d ",wapi_len);
		...
		while(len >= 1) {
			...
			switch(section) {
				...
				case WEXT_CSCAN_NPROBE_SECTION:
					DBG_8723A("WEXT_CSCAN_NPROBE_SECTION\n");
					break;
				#endif
...
#ifdef CONFIG_WAPI_SUPPORT
#ifndef CONFIG_IOCTL_CFG80211
		DBG_8723A("rtw_wx_set_auth: IW_AUTH_KEY_MGMT case \n");
		if(value == IW_AUTH_KEY_MGMT_WAPI_PSK)
			padapter->wapiInfo.bWapiPSK = true;
		else
			padapter->wapiInfo.bWapiPSK = false;
		DBG_8723A("rtw_wx_set_auth: IW_AUTH_KEY_MGMT bwapipsk %d \n",padapter->wapiInfo.bWapiPSK);
#endif
#endif
		/*
...
*/
	...
	//DBG_8723A("enc_alg=0x%x\n", psecuritypriv->dot11PrivacyAlgrthm);
	//DBG_8723A("auth_type=0x%x\n", psecuritypriv->ndisauthtype);
	//DBG_8723A("enc_type=0x%x\n", psecuritypriv->ndisencryptstatus);
...
static void rtw_dbg_mode_hdl(_adapter *padapter, u32 id, u8 *pdata, u32 len)
{
	...
	switch(id)
	{
		case GEN_MP_IOCTL_SUBCODE(MP_START):
			DBG_8723A("871x_driver is only for normal mode, can't enter mp mode\n");
			break;
		...
		case GEN_MP_IOCTL_SUBCODE(WRITE_RF_REG):
			...
			DBG_8723A("==> trigger gpio 0\n");
			rtw_hal_set_hwreg(padapter, HW_VAR_TRIGGER_GPIO_0, 0);
			break;	
#ifdef CONFIG_BT_COEXIST
		case GEN_MP_IOCTL_SUBCODE(SET_DM_BT):			
			DBG_8723A("==> set dm_bt_coexist:%x\n",*(u8 *)pdata);
			rtw_hal_set_hwreg(padapter, HW_VAR_BT_SET_COEXIST, pdata);
			break;
		case GEN_MP_IOCTL_SUBCODE(DEL_BA):
			DBG_8723A("==> delete ba:%x\n",*(u8 *)pdata);
...
{	
	...
	if (phandler->handler)
	{
		...
		DBG_8723A("rtw_mp_ioctl_hdl(): err!, subcode=%d, oid=%d, handler=%p\n", 
                               ...
                               union iwreq_data *wrqu, char *extra)
{
	...
	{
		DBG_8723A( "[%s] Unknown WPS config methodn", __FUNCTION__ );
		return( ret );
	}
...
void mac_reg_dump(_adapter *padapter)
{
	...
	{	
		if(j%4==1)	DBG_8723A("0x%02x",i);
		DBG_8723A(" 0x%08x ",rtw_read32(padapter,i));		
		if((j++)%4 == 0)	DBG_8723A("\n");	
	}
...
//		DBG_8723A("\n");
	...
	else if (strcmp(tmp[0], "realmap") == 0)
	{
		...
		{
			DBG_8723A("%s: read realmap Fail!!\n", __FUNCTION__);
			err = -EFAULT;
			goto exit;
		}

//		DBG_8723A("OFFSET\tVALUE(hex)\n");
		sprintf(extra, "\n");
		for (i = 0; i < EFUSE_MAP_SIZE; i += 16)
		{
//			DBG_8723A("0x%02x\t", i);
...
//		DBG_8723A("\n");
	...
	else if (strcmp(tmp[0], "rmap") == 0)
	{
		...
		{
			DBG_8723A("%s: rmap Fail!! Parameters error!\n", __FUNCTION__);
			err = -EINVAL;
			goto exit;
		}
		...
		if (cnts == 0)
		{
			DBG_8723A("%s: rmap Fail!! cnts error!\n", __FUNCTION__);
			err = -EINVAL;
			goto exit;
		}
		DBG_8723A("%s: cnts=%d\n", __FUNCTION__, cnts);
		...
		{
			DBG_8723A("%s: addr(0x%X)+cnts(%d) parameter error!\n", __FUNCTION__, addr, cnts);
			err = -EINVAL;
			goto exit;
		}
		...
		{
			DBG_8723A("%s: rtw_efuse_map_read error!\n", __FUNCTION__);
			err = -EFAULT;
			goto exit;
		}
...
//		DBG_8723A("}\n");
	...
	else if (strcmp(tmp[0], "realraw") == 0)
	{
		...
		{
			DBG_8723A("%s: rtw_efuse_access Fail!!\n", __FUNCTION__);
			err = -EFAULT;
			goto exit;
		}
...
//		DBG_8723A("}\n");
	...
	else if (strcmp(tmp[0], "mac") == 0)
	{
		...
		EFUSE_GetEfuseDefinition(padapter, EFUSE_WIFI, TYPE_AVAILABLE_EFUSE_BYTES_TOTAL, (PVOID)&max_available_size, _FALSE);
		if ((addr + cnts) > max_available_size) {
			DBG_8723A("%s: addr(0x%02x)+cnts(%d) parameter error!\n", __FUNCTION__, addr, cnts);
			err = -EFAULT;
			goto exit;
		}
...
//		DBG_8723A("%s: MAC address={", __FUNCTION__);
		*extra = 0;
		for (i=0; i<cnts; i++)
		{
//			DBG_8723A("%02X", data[i]);
...
//		DBG_8723A("}\n");
	...
	else if (strcmp(tmp[0], "vidpid") == 0)
	{
		...
		if (rtw_efuse_map_read(padapter, addr, cnts, data) == _FAIL)
		{
			DBG_8723A("%s: rtw_efuse_access error!!\n", __FUNCTION__);
			err = -EFAULT;
			goto exit;
		}

//		DBG_8723A("%s: {VID,PID}={", __FUNCTION__);
		*extra = 0;
		for (i=0; i<cnts; i++)
		{
//			DBG_8723A("0x%02x", data[i]);
...
//		DBG_8723A("}\n");
	...
	else if (strcmp(tmp[0], "btfmap") == 0)
	{
		...
		{
			DBG_8723A("%s: rtw_BT_efuse_map_read Fail!!\n", __FUNCTION__);
			err = -EFAULT;
			goto exit;
		}
...
//			DBG_8723A("0x%03x\t", i);
			...
			for (j=0; j<8; j++) {
//				DBG_8723A("%02X ", pEfuseHal->BTEfuseInitMap[i+j]);
...
//		DBG_8723A("\n");
	...
	else if (strcmp(tmp[0],"btrmap") == 0)
	{
		...
		// rmap addr cnts
		addr = simple_strtoul(tmp[1], &ptmp, 16);
		DBG_8723A("%s: addr=0x%X\n", __FUNCTION__, addr);

		cnts = simple_strtoul(tmp[2], &ptmp, 10);
		if (cnts == 0)
		{
			DBG_8723A("%s: btrmap Fail!! cnts error!\n", __FUNCTION__);
			err = -EINVAL;
			goto exit;
		}
		DBG_8723A("%s: cnts=%d\n", __FUNCTION__, cnts);
		...
		{
			DBG_8723A("%s: rtw_BT_efuse_map_read error!!\n", __FUNCTION__);
			err = -EFAULT;
			goto exit;
		}

		*extra = 0;
//		DBG_8723A("%s: bt efuse data={", __FUNCTION__);
		for (i=0; i<cnts; i++)
		{
//			DBG_8723A("0x%02x ", data[i]);
...
//			DBG_8723A("0x%03x\t", i);
			sprintf(extra, "%s0x%03x\t", extra, i);
			for (j=0; j<8; j++) {
//				DBG_8723A("%02X ", pEfuseHal->fakeBTEfuseModifiedMap[i+j]);
...
//			DBG_8723A("\t0x%02x\t", i);
			sprintf(extra, "%s0x%02x\t", extra, i);
			for (j=0; j<8; j++) {
//				DBG_8723A("%02X ", pEfuseHal->fakeEfuseModifiedMap[i+j]);
			...
			union iwreq_data *wdata, char *extra)
{
	...
	if (strcmp(tmp[0], "wmap") == 0)
	{
		...
		{
			DBG_8723A("%s: rtw_efuse_map_write error!!\n", __FUNCTION__);
			err = -EFAULT;
			goto exit;
		}
	}
	...
	else if (strcmp(tmp[0], "mac") == 0)
	{
		...
		if (cnts > 6)
		{
			DBG_8723A("%s: error data for mac addr=\"%s\"\n", __FUNCTION__, tmp[1]);
			err = -EFAULT;
			goto exit;
		}
    ...
    else if (strcmp(tmp[0], "wldumpfake") == 0)
	{
		if (rtw_efuse_map_read(padapter, 0, EFUSE_MAP_SIZE,  pEfuseHal->fakeEfuseModifiedMap) == _SUCCESS) {
			DBG_8723A("%s: WiFi hw efuse dump to Fake map success \n", __FUNCTION__); 
		} else {
			DBG_8723A("%s: WiFi hw efuse dump to Fake map Fail \n", __FUNCTION__);
			err = -EFAULT;
		}
	}
	else if (strcmp(tmp[0], "btwmap") == 0)
	{
		...
		{
			DBG_8723A("%s: rtw_BT_efuse_map_write error!!\n", __FUNCTION__);
			err = -EFAULT;
			goto exit;
		}
	}
	...
	else if (strcmp(tmp[0], "btdumpfake") == 0)
	{
		if (rtw_BT_efuse_map_read(padapter, 0, EFUSE_BT_MAX_MAP_LEN, pEfuseHal->fakeBTEfuseModifiedMap) == _SUCCESS) {
			DBG_8723A("%s: BT read all map success\n", __FUNCTION__);
		} else {
			DBG_8723A("%s: BT read all map Fail!\n", __FUNCTION__);
			err = -EFAULT;
		}
	}
	...
	else if (strcmp(tmp[0], "wlfk2map") == 0)
	{
		...
		{
			DBG_8723A("%s: rtw_efuse_map_write fakeEfuseModifiedMap error!\n", __FUNCTION__);
			err = -EFAULT;
			goto exit;
		}
	}
			...
			struct iw_point *wrqu, char *extra)
{
	...
	{
		#ifdef CONFIG_RTL8723A
		DBG_8723A("_rtw_mp_xmit_priv for Download BT patch FW\n");
			...
			struct iw_point *wrqu, char *extra)
{
	...
	if(padapter->registrypriv.mp_mode ==1)
	{
		...
		DBG_8723A("rtw_mp_stop -> registrypriv.mp_mode=%d ",padapter->registrypriv.mp_mode);
			...
			struct iw_point *wrqu, char *extra)
{
	...
	//DBG_8723A("%s: tx_pwr_idx_a=%x b=%x\n", __func__, idx_a, idx_b);
			...
			struct iw_point *wrqu, char *extra)
{
	...
	//DBG_8723A("%s: antenna=0x%x\n", __func__, antenna);		
	padapter->mppriv.antenna_tx = antenna;
	//DBG_8723A("%s:mppriv.antenna_rx=%d\n", __func__, padapter->mppriv.antenna_tx);
			...
			struct iw_point *wrqu, char *extra)
{
	...
	//DBG_8723A("%s: count=%d countPkTx=%d cotuTx=%d CarrSprTx=%d scTx=%d sgleTx=%d pkTx=%d stop=%d\n", __func__, count, countPkTx, cotuTx, CarrSprTx, pkTx, sgleTx, scTx, stop);
			...
			struct iw_point *wrqu, char *extra)
{
	...
	//DBG_8723A("print extra %s \n",extra); 
	 ...
	 if( bwrite == 0 )	
	 {
		 //DBG_8723A("to write val:%d",val);
			EFUSE_GetEfuseDefinition(padapter, EFUSE_WIFI, TYPE_AVAILABLE_EFUSE_BYTES_TOTAL, (PVOID)&max_available_size, _FALSE);
			if( 2 > max_available_size )
			{			
				DBG_8723A("no available efuse!\n");
				return -EFAULT;
			}	
			if ( rtw_efuse_map_write(padapter, addr, cnt, &val) == _FAIL )
			{
				DBG_8723A("rtw_efuse_map_write error \n");			
				return -EFAULT;
			} 
			else
			{
			...
			union iwreq_data *wrqu, char *extra)
{
	...
	if(bMain==0)
	{
		...
		DBG_8723A("%s:PHY_SetRFPathSwitch=TRUE\n", __func__);	
	}
	else if(bTurnoff==0)
	{
		MP_PHY_SetRFPathSwitch(padapter,_FALSE);
		DBG_8723A("%s:PHY_SetRFPathSwitch=FALSE\n", __func__);	
	}
			...
			union iwreq_data *wrqu, char *extra)
{
	...
	if ( strncmp(extra, "dlfw", 4) == 0)
	{
		...
		{
			//pMptCtx->h2cReqNum=0;
			DBG_8723A("padapter->bBTFWReady == _FALSE rtl8723a_FirmwareDownload  !\n");
		...
		else
		{
			...
			{
				DBG_8723A("FirmwareDownloadBT ready = 0x%x ", pMptCtx->mptOutBuf[i]);
	...
	if ( strncmp(extra, "down", 4) == 0){
		DBG_8723A("SetBT down for to hal_init !\n");
	...
	else
	{
		...
		for (jj=0, kk=0; jj<cnts; jj++, kk+=2)
		{
				...
				DBG_8723A("BtReq.pParamStart[%d]=%x \n",ii,BtReq.pParamStart[jj]);
		}
	}
	...
	DBG_8723A("%s: BtReq.paraLength =%d\n", __FUNCTION__, BtReq.paraLength);
		
	DBG_8723A("opCodeVer=%d,OpCode=%d \n",BtReq.opCodeVer,BtReq.OpCode);
	...
	{
		DBG_8723A("%s: BtReq.pParamStart[ %d ] = 0x%02x \n", __func__,i,BtReq.pParamStart[i]); 
	}
...
todo:
	...
	for (i=4; i<pMptCtx->mptOutLen; i++)
	{
		DBG_8723A("0x%x ", pMptCtx->mptOutBuf[i]);
			...
			union iwreq_data *wdata, char *extra)
{
	...
	{
	case MP_START:
			DBG_8723A("set case mp_start \n");
			rtw_mp_start (dev,info,wrqu,extra);
			 break; 
			 
	case MP_STOP:
			DBG_8723A("set case mp_stop \n");
			rtw_mp_stop (dev,info,wrqu,extra);
			 break; 
			 
	case MP_BANDWIDTH:
			DBG_8723A("set case mp_bandwidth \n");
			rtw_mp_bandwidth (dev,info,wrqu,extra);
			break;
				
	case MP_RESET_STATS:
			DBG_8723A("set case MP_RESET_STATS \n");
			rtw_mp_reset_stats	(dev,info,wrqu,extra);
			break;
	case MP_SetRFPathSwh:		
			DBG_8723A("set MP_SetRFPathSwitch \n");
			rtw_mp_SetRFPath  (dev,info,wdata,extra);
			break;
	case CTA_TEST:
			DBG_8723A("set CTA_TEST\n");
			rtw_cta_test_start (dev, info, wdata, extra);
			break;
	case MP_DISABLE_BT_COEXIST:
			DBG_8723A("set case MP_DISABLE_BT_COEXIST \n");
			...
			union iwreq_data *wdata, char *extra)
{
	...
	case MP_PHYPARA:
			DBG_8723A("mp_get  MP_PHYPARA \n");
			rtw_mp_phypara(dev,info,wrqu,extra);	
			break;

	case MP_CHANNEL:
			DBG_8723A("set case mp_channel \n");
			rtw_mp_channel (dev,info,wrqu,extra);
			break;
			
	case READ_REG:
			DBG_8723A("mp_get  READ_REG \n");
			rtw_mp_read_reg (dev,info,wrqu,extra);
			 break; 
	case READ_RF:
			DBG_8723A("mp_get  READ_RF \n");
			rtw_mp_read_rf (dev,info,wrqu,extra);
			break; 
			
	case MP_RATE:
			DBG_8723A("set case mp_rate \n");
			rtw_mp_rate (dev,info,wrqu,extra);
			break;
			
	case MP_TXPOWER:
			DBG_8723A("set case MP_TXPOWER \n");
			rtw_mp_txpower (dev,info,wrqu,extra);
			break;
			
	case MP_ANT_TX:
			DBG_8723A("set case MP_ANT_TX \n");
			rtw_mp_ant_tx (dev,info,wrqu,extra);
			break;
			
	case MP_ANT_RX:
			DBG_8723A("set case MP_ANT_RX \n");
			rtw_mp_ant_rx (dev,info,wrqu,extra);
			break;
			
	case MP_QUERY:
			//DBG_8723A("mp_get mp_query MP_QUERY \n");
			rtw_mp_trx_query(dev,info,wrqu,extra);
			break;
					
	case MP_CTX:
			DBG_8723A("set case MP_CTX \n");
			rtw_mp_ctx (dev,info,wrqu,extra);
			break;
			
	case MP_ARX:
			DBG_8723A("set case MP_ARX \n");
			rtw_mp_arx (dev,info,wrqu,extra);
			break;
			
	case EFUSE_GET:
			DBG_8723A("efuse get EFUSE_GET \n");
			rtw_mp_efuse_get(dev,info,wdata,extra);
		 break; 
		 
	case MP_DUMP:
			DBG_8723A("set case MP_DUMP \n");
			rtw_mp_dump (dev,info,wrqu,extra);
		 break; 
	case MP_PSD:
			DBG_8723A("set case MP_PSD \n");
			rtw_mp_psd (dev,info,wrqu,extra);
		 break;
	case MP_THER:
			DBG_8723A("set case MP_THER \n");
			rtw_mp_thermal (dev,info,wrqu,extra);
		break;
	case MP_PwrCtlDM:
			DBG_8723A("set MP_PwrCtlDM\n");
			rtw_mp_PwrCtlDM (dev,info,wrqu,extra);
		break;
	case MP_QueryDrvStats:		
			DBG_8723A("mp_get MP_QueryDrvStats \n");
			rtw_mp_QueryDrv  (dev,info,wdata,extra);
			break;
		case MP_PWRTRK:
			DBG_8723A("set case MP_PWRTRK \n");
			rtw_mp_pwrtrk (dev,info,wrqu,extra);
			break;			 
	case EFUSE_SET:
			DBG_8723A("set case efuse set \n");
			rtw_mp_efuse_set (dev,info,wdata,extra);
			break;			 

#ifdef CONFIG_RTL8723A			
	case MP_SetBT:		
			DBG_8723A("set MP_SetBT \n");
...
thread_return lbk_thread(thread_context context)
{
	...
	do {
		...
		cnt++;
		DBG_8723A("%s: wirte port cnt=%d size=%d\n", __func__, cnt, ploopback->txsize);
