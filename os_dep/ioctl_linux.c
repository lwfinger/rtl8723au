...
static int wpa_set_encryption(struct net_device *dev, struct ieee_param *param, u32 param_len)
{
	...
	if (strcmp(param->u.crypt.alg, "WEP") == 0) {
		...
		if (wep_key_len > 0) {
			...
			pwep = (struct ndis_802_11_wep	 *) kzalloc(wep_total_len, GFP_KERNEL);
			if (pwep == NULL) {
		...
		if (param->u.crypt.set_tx) {
			...
			if (wep_key_idx >= WEP_KEYS) {
		      ...
		      psecuritypriv->dot11DefKeylen[wep_key_idx] =
			      pwep->KeyLength;
                               ...
                               union iwreq_data *wrqu, char *extra)
{
	...
	if (padapter->bShowGetP2PState)
		DBG_8723A("[%s] Role = %d, Status = %d, peer addr = %pM\n", __func__,
                               ...
                               union iwreq_data *wrqu, char *extra)
{
	...
	if (uintPeerChannel) {
		...
		if (!pwfd_info->peer_session_avail) {
	...
	list_for_each_safe(plist, ptmp, phead) {
		...
		if ((p2pie = rtw_get_p2p_ie(&pnetwork->network.IEs[12], pnetwork->network.IELength - 12, NULL, &p2pielen))) {
			while (p2pie) {
				...
				/* Get the next P2P IE */
				p2pie = rtw_get_p2p_ie(p2pie+p2pielen, pnetwork->network.IELength -
						       12 -(p2pie -&pnetwork->network.IEs[12] + p2pielen),
						       NULL, &p2pielen);
			}
		}
	}
...
/* This function is used to inform the driver the user had specified
 * the pin code value or pbc
 * to application.
 */
                               ...
                               union iwreq_data *wrqu, char *extra)
{
	 ...
	 * if the input data is P2P_GOT_WPSINFO_PBC -> the utility just
	 *   determine to use the PBC
	 */
	if (*extra == '0')
...
static int set_group_key(struct rtw_adapter *padapter, u8 *key,
			 u8 alg, int keyid)
{
	u8 keylen;
	struct cmd_obj* pcmd;
...
static int set_wep_key(struct rtw_adapter *padapter, u8 *key,
		       u8 keylen, int keyid)
{
	u8 alg;
...
static int rtw_set_encryption(struct net_device *dev,
...
static int rtw_set_beacon(struct net_device *dev,
...
static int rtw_add_sta(struct net_device *dev, struct ieee_param *param)
{
	...
	DBG_8723A("rtw_add_sta(aid =%d) = %pM\n", param->u.add_sta.aid, param->sta_addr);
	...
	if (psta) {
		...
		if (WLAN_STA_HT&flags) {
			...
			psta->qos_option = 1;
			memcpy((void *)&psta->htpriv.ht_cap,
...
static int rtw_ioctl_get_sta_data(struct net_device *dev, struct ieee_param *param, int len)
{
	...
	DBG_8723A("rtw_ioctl_get_sta_info, sta_addr: %pM\n",
		  param_ex->sta_addr);
...
static int rtw_set_hidden_ssid(struct net_device *dev, struct ieee_param *param, int len)
{
	...
	if (ssid_ie && ssid_len) {
		...
		ssid[ssid_len > IEEE80211_MAX_SSID_LEN ?
		     IEEE80211_MAX_SSID_LEN : ssid_len] = 0x0;
