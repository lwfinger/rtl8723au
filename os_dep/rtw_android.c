...
static int wl_android_set_pno_setup(struct net_device *dev, char *command, int total_len)
{
	...
	if (total_len < (strlen(CMD_PNOSETUP_SET) + sizeof(cmd_tlv_t))) {
		DBG_8723A("%s argument=%d less min size\n", __FUNCTION__, total_len);
		goto exit_proc;
	}
	...
	if ((cmd_tlv_temp->prefix == PNO_TLV_PREFIX) &&
		...
		if ((nssid = wl_iw_parse_ssid_list_tlv(&str_ptr, ssids_local,
			...
			DBG_8723A("SSID is not presented or corrupted ret=%d\n", nssid);
			goto exit_proc;
		} else {
			if ((str_ptr[0] != PNO_TLV_TYPE_TIME) || (tlv_size_left <= 1)) {
				DBG_8723A("%s scan duration corrupted field size %d\n",
			...
			if (str_ptr[0] != 0) {
				if ((str_ptr[0] != PNO_TLV_FREQ_REPEAT)) {
					DBG_8723A("%s pno repeat : corrupted field\n",
						__FUNCTION__);
					goto exit_proc;
				}
				str_ptr++;
				...
				if (str_ptr[0] != PNO_TLV_FREQ_EXPO_MAX) {
					DBG_8723A("%s FREQ_EXPO_MAX corrupted field size\n",
						__FUNCTION__);
					goto exit_proc;
				}
				str_ptr++;
		...
		DBG_8723A("%s get wrong TLV command\n", __FUNCTION__);
		goto exit_proc;
	}
...
int rtw_android_priv_cmd(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	//DBG_8723A("%s priv_cmd.buf=%p priv_cmd.total_len=%d  priv_cmd.used_len=%d\n",__func__,priv_cmd.buf,priv_cmd.total_len,priv_cmd.used_len);
	command = rtw_zmalloc(priv_cmd.total_len);
	if (!command)
	{
		DBG_8723A("%s: failed to allocate memory\n", __FUNCTION__);
		ret = -ENOMEM;
		goto exit;
	}

	if (!access_ok(VERIFY_READ, priv_cmd.buf, priv_cmd.total_len)){
		DBG_8723A("%s: failed to access memory\n", __FUNCTION__);
		ret = -EFAULT;
		goto exit;
	 }
	...
	DBG_8723A("%s: Android private cmd \"%s\" on %s\n"
	...
	if (!g_wifi_on) {
		DBG_8723A("%s: Ignore private cmd \"%s\" - iface %s is down\n"
			,__FUNCTION__, command, ifr->ifr_name);
		ret = 0;
		goto exit;
	}
...
#endif
	default:
		DBG_8723A("Unknown PRIVATE command %s - ignored\n", command);
...
int rtw_android_wifictrl_func_add(void)
{
	...
	ret = wifi_add_dev();
	if (ret) {
		DBG_8723A("%s: platform_driver_register failed\n", __FUNCTION__);
		return ret;
	}
	...
	if (down_timeout(&wifi_control_sem,  msecs_to_jiffies(1000)) != 0) {
		ret = -EINVAL;
		DBG_8723A("%s: platform_driver_register timeout\n", __FUNCTION__);
	}
...
void *wl_android_prealloc(int section, unsigned long size)
{
	...
	if (wifi_control_data && wifi_control_data->mem_prealloc) {
		...
		if (alloc_ptr) {
			DBG_8723A("success alloc section %d\n", section);
			if (size != 0L)
	...
	DBG_8723A("can't alloc section %d\n", section);
	return NULL;
}
...
static int wifi_add_dev(void)
{
	DBG_8723A("## Calling platform_driver_register\n");
...
static void wifi_del_dev(void)
{
	DBG_8723A("## Unregister platform_driver_register\n");
