...
/*  */
void
phy_ConfigBBExternalPA(struct rtw_adapter *Adapter)
{
	struct hal_data_8723a *pHalData = GET_HAL_DATA(Adapter);
	u16 i = 0;
	u32 temp = 0;
	...
	/*  2010/10/19 MH According to Jenyu/EEChou 's opinion, we need not to
	    execute the same code as SU. It is already updated in
	    PHY_REG_1T_HP.txt. */
}
...
int
PHY_ConfigRFExternalPA(struct rtw_adapter *Adapter, RF_RADIO_PATH_E eRFPath)
{
	int rtStatus = _SUCCESS;
	struct hal_data_8723a *pHalData = GET_HAL_DATA(Adapter);
	u16 i = 0;
	...
	/*  2010/10/19 MH According to Jenyu/EEChou 's opinion, we need not
	    to execute the */
 ...
 *
 * Overview:    This function is write register and then readback to make
 *		sure whether BB[PHY0, PHY1], RF[Patha, path b, path c, path d]
 *		is Ok
 *
 * Input:
 *	struct rtw_adapter *Adapter
 ...
 *---------------------------------------------------------------------------*/
int
PHY_CheckBBAndRFOK(struct rtw_adapter *Adapter, HW90_BLOCK_E CheckBlock,
		   RF_RADIO_PATH_E eRFPath)
{
	...
	for (i = 0 ; i < CheckTimes ; i++) {
		...
		case HW90_BLOCK_PHY1:
			rtw_write32(Adapter, WriteAddr[CheckBlock],
				    WriteData[i]);
			ulRegRead = rtw_read32(Adapter, WriteAddr[CheckBlock]);
			break;
		case HW90_BLOCK_RF:
			/*  When initialization, we want the delay
			    function(delay_ms(), delay_us() */
			/*  ==> actually we call PlatformStallExecution())
			    to do NdisStallExecution() */
			/*  [busy wait] instead of NdisMSleep(). So we acquire
			    RT_INITIAL_SPINLOCK */
			/*  to run at Dispatch level to achive it. */
			/* cosa PlatformAcquireSpinLock(Adapter,
			   RT_INITIAL_SPINLOCK); */
			WriteData[i] &= 0xfff;
			PHY_SetRFReg(Adapter, eRFPath, WriteAddr[HW90_BLOCK_RF],
				     bRFRegOffsetMask, WriteData[i]);
			/*  TODO: we should not delay for such a long time.
			    Ask SD3 */
			mdelay(10);
			ulRegRead = PHY_QueryRFReg(Adapter, eRFPath,
						   WriteAddr[HW90_BLOCK_RF],
						   bMaskDWord);
			mdelay(10);
			/* cosa PlatformReleaseSpinLock(Adapter,
			   RT_INITIAL_SPINLOCK); */
			break;
...
/*  */
int
phy_TxPwrIdxToDbm(struct rtw_adapter *Adapter, WIRELESS_MODE WirelessMode,
		  u8 TxPwrIdx)
{
	int Offset = 0;
	int PwrOutDbm = 0;
	...
	/*  */
	/*  Tested by MP, we found that CCK Index 0 equals to -7dbm,
	    OFDM legacy equals to -8dbm. */
	/*  Note: */
	/*	The mapping may be different by different NICs.
		Do not use this formula for what needs accurate result. */
	/*  By Bruce, 2008-01-29. */
...
static	bool phy_SwChnlStepByStep(struct rtw_adapter *Adapter,
...
static	bool
phy_SetSwChnlCmdArray(SwChnlCmd*CmdTable, u32 CmdTableIdx, u32 CmdTableSz,
		      SwChnlCmdID CmdID, u32 Para1, u32 Para2, u32 msDelay)
{
	SwChnlCmd* pCmd;

	if (!CmdTable) {
		/* RT_ASSERT(false, ("phy_SetSwChnlCmdArray():
	...
	if (CmdTableIdx >= CmdTableSz) {
		...
		/*		("phy_SetSwChnlCmdArray(): Access invalid "
				"index, please check size of the table, "
				"CmdTableIdx:%ld, CmdTableSz:%ld\n", */
...
static void
phy_FinishSwChnlNow(struct rtw_adapter *Adapter, u8 channel)
{
}
...
*/
static void _PHY_SetRFPathSwitch(struct rtw_adapter *pAdapter, bool bMain,
				 bool is2T)
{
	u8 u1bTmp;
	...
	if (is2T) {
		if(bMain)
			PHY_SetBBReg(pAdapter, rFPGA0_XB_RFInterfaceOE,
		...
		if(bMain)
			PHY_SetBBReg(pAdapter, rFPGA0_XA_RFInterfaceOE,
				     0x300, 0x2);	/* Main */
		else
			PHY_SetBBReg(pAdapter, rFPGA0_XA_RFInterfaceOE,
				     0x300, 0x1);	/* Aux */
...
static bool _PHY_QueryRFPathSwitch(struct rtw_adapter *pAdapter, bool is2T)
{
/*	if(is2T) */
/*		return true; */
	...
	if (is2T) {
		if (PHY_QueryBBReg(pAdapter, rFPGA0_XB_RFInterfaceOE,
				   BIT5 | BIT6) == 0x01)
			return true;
		else
			return false;
	} else {
		if (PHY_QueryBBReg(pAdapter,
				   rFPGA0_XA_RFInterfaceOE, 0x300) == 0x02)
			return true;
		else
			return false;
	}
...
_PHY_DumpRFReg(struct rtw_adapter *pAdapter)
{
	...
	for (rfRegOffset = 0x00;rfRegOffset<=0x30;rfRegOffset++)
		rfRegValue = PHY_QueryRFReg(pAdapter,RF_PATH_A,
					    rfRegOffset, bMaskDWord);
}
