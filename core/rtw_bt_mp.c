...
BT_CTRL_STATUS mptbt_CheckC2hFrame(PADAPTER Adapter, PBT_H2C pH2c, PBT_EXT_C2H pExtC2h)
{
...
BT_CTRL_STATUS mptbt_SendH2c(PADAPTER Adapter, PBT_H2C pH2c, u2Byte h2cCmdLen)
{
...
BT_CTRL_STATUS mptbt_CheckBtRspStatus(PADAPTER Adapter, PBT_EXT_C2H pExtC2h)
{
...
BT_CTRL_STATUS mptbt_BtFwOpCodeProcess(PADAPTER Adapter, u1Byte btFwOpCode,
				       u1Byte opCodeVer, pu1Byte pH2cPar,
				       u1Byte h2cParaLen)
{
...
u2Byte mptbt_BtReady(PADAPTER Adapter, PBT_REQ_CMD pBtReq, PBT_RSP_CMD pBtRsp)
{
...
void MPTBT_FwC2hBtMpCtrl(PADAPTER Adapter, pu1Byte tmpBuf, u1Byte length)
{
	u32 i;
	PMPT_CONTEXT	pMptCtx = &(Adapter->mppriv.MptCtx);
...
u2Byte mptbt_BtTestCtrl(PADAPTER Adapter, PBT_REQ_CMD pBtReq, PBT_RSP_CMD pBtRsp)
{
	u1Byte		h2cParaBuf[6] = {0};
	u1Byte		h2cParaLen = 0;
	u2Byte		paraLen = 0;
...
u2Byte mptbt_TestBT(IN	PADAPTER Adapter, PBT_REQ_CMD pBtReq, PBT_RSP_CMD pBtRsp)
{
	u1Byte		h2cParaBuf[6] = {0};
	u1Byte		h2cParaLen = 0;
	u2Byte		paraLen = 0;
...
void mptbt_BtControlProcess(PADAPTER Adapter, void *pInBuf)
{
	u1Byte		H2C_Parameter[6] = {0};
	PBT_H2C		pH2c = (PBT_H2C)&H2C_Parameter[0];
	PMPT_CONTEXT	pMptCtx = &(Adapter->mppriv.MptCtx);
	...
	memset(&pMptCtx->mptOutBuf[0], '\0', 100);
