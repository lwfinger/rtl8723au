...
void MPh2c_timeout_handle(void *FunctionContext)
{
	...
	DBG_8723A("[MPT], MPh2c_timeout_handle \n");
	...
	)
{
	...
	DBG_8723A("[MPT], mptbt_SendH2c()=========>\n");

	for(i=0; i<BT_H2C_MAX_RETRY; i++)
	{
		DBG_8723A("[MPT], Send H2C command to wifi!!!\n");
		...
		{
			DBG_8723A("[MPT], Received WiFi MptH2cRspEvent!!!\n");
			if(WaitC2Hevent(Adapter, &pMptCtx->MptBtC2hEvent, 400))
			{
				DBG_8723A("[MPT], Received MptBtC2hEvent!!!\n");
				break;
			}
			else
			{
				DBG_8723A("[MPT], Error!!BT MptBtC2hEvent timeout!!\n");
				h2cStatus = BT_STATUS_H2C_BT_NO_RSP;
			}
		}
		else
		{
			DBG_8723A("[MPT], Error!!WiFi  MptH2cRspEvent timeout!!\n");
	...
	)
{
	...
	DBG_8723A("[MPT], pH2c->opCode=%d\n", pH2c->opCode);
	DBG_8723A("[MPT], pH2c->opCodeVer=%d\n", pH2c->opCodeVer);
	DBG_8723A("[MPT], pH2c->reqNum=%d\n", pH2c->reqNum);
	DBG_8723A("[MPT], h2c parameter length=%d\n", h2cParaLen);
	if(h2cParaLen)
	{
		DBG_8723A("[MPT], parameters(hex): \n");
	...
	if(BT_STATUS_H2C_SUCCESS == h2cStatus)
	{
		...
		{
			DBG_8723A("[MPT], Error!! C2H failed for pH2c->opCode=%d\n", pH2c->opCode);
	...
	{
		DBG_8723A("[MPT], Error!! H2C failed for pH2c->opCode=%d\n", pH2c->opCode);
	...
	)
{
	...
	{
		DBG_8723A("[MPT], Error!! Upper OP code version not match!!!\n");
	...
	if(BT_STATUS_BT_OP_SUCCESS != retStatus)
	{
		...
		DBG_8723A("[MPT], Error!! status code=0x%x \n", pBtRsp->status);
		return paraLen;
	}
	else
	{
		...
		DBG_8723A("[MPT], btRealFwVer=0x%x, btFwVer=0x%x\n", btRealFwVer, btFwVer);
	}
	...
	DBG_8723A("[MPT], Local BDAddr:");
	for(i=0; i<6; i++)
	{
		DBG_8723A(" 0x%x ", bdAddr[i]);
	}
	...
	)
{
	...
	if(1 == pBtReq->paraLength)
	{
		...
		DBG_8723A("[MPT], BtTestMode=%d \n", btModeToSet);
	}
	else
	{
		DBG_8723A("[MPT], Error!! wrong parameter length=%d (should be 1)\n", pBtReq->paraLength);
	...
	)
{
	...
	{
		DBG_8723A("[MPT], %s,bBTFWReady == _FALSE\n",__func__);
		return;
	}
	/* cancel_timeout for h2c handle */
	_cancel_timer_ex( &pMptCtx->MPh2c_timeout_timer);

	DBG_8723A("[MPT], MPTBT_FwC2hBtMpCtrl(), hex: \n");
	for(i=0;i<=length;i++)
	{
		/* DBG_8723A("[MPT], MPTBT_FwC2hBtMpCtrl(), hex: \n",tmpBuf[i], length); */
		DBG_8723A(" 0x%x ",tmpBuf[i]);
	}
	...
	switch(pExtC2h->extendId)
	{
		case EXT_C2H_WIFI_FW_ACTIVE_RSP:
			DBG_8723A("[MPT], EXT_C2H_WIFI_FW_ACTIVE_RSP\n");
			DBG_8723A("[MPT], pExtC2h->buf hex: \n");
			if( length > 32 || length < 3 )
					break ;
			for(i=0;i<=(length-3);i++)
				DBG_8723A(" 0x%x ",pExtC2h->buf[i]);
				/* PlatformSetEvent(&pMptCtx->MptH2cRspEvent); */
				pMptCtx->MptH2cRspEvent=_TRUE;
				_rtw_up_sema(&pMptCtx->MPh2c_Sema);
			break;
		case EXT_C2H_TRIG_BY_BT_FW:
			DBG_8723A("[MPT], EXT_C2H_TRIG_BY_BT_FW\n");
				/* PlatformMoveMemory(&pMptCtx->c2hBuf[0], tmpBuf, length); */
			_rtw_memcpy(&pMptCtx->c2hBuf[0], tmpBuf, length);
			DBG_8723A("[MPT], pExtC2h->statusCode=0x%x\n", pExtC2h->statusCode);
			DBG_8723A("[MPT], pExtC2h->retLen=0x%x\n", pExtC2h->retLen);
			DBG_8723A("[MPT], pExtC2h->opCodeVer=0x%x\n", pExtC2h->opCodeVer);
			DBG_8723A("[MPT], pExtC2h->reqNum=0x%x\n", pExtC2h->reqNum);
			DBG_8723A("[MPT], pExtC2h->buf hex: \n");
			for(i=0;i<=(length-3);i++)
				DBG_8723A(" 0x%x ",pExtC2h->buf[0]);
	...
	)
{
	...
	{
		DBG_8723A("[MPT], Error!! wrong parameter length=%d (should larger than 1)\n", pBtReq->paraLength);
		pBtRsp->status = BT_STATUS_PARAMETER_FORMAT_ERROR_U;
		return paraLen;
	}
	...
	DBG_8723A("[MPT], getType=%d, getParaLen=%d\n", getType, getParaLen);

	/*  check parameter first */
	switch(getType)
	{
		case BT_GGET_REG:
			DBG_8723A("[MPT], [BT_GGET_REG]\n");
			validParaLen = 5;
			if(getParaLen == validParaLen)
			{
				...
				regAddr = *pu4Tmp;
				DBG_8723A("[MPT], BT_GGET_REG regType=0x%x, regAddr=0x%x!!\n",
					regType, regAddr);
			...
			break;
		case BT_GGET_STATUS:
			DBG_8723A("[MPT], [BT_GGET_STATUS]\n");
			validParaLen = 0;
			break;
		case BT_GGET_REPORT:
			DBG_8723A("[MPT], [BT_GGET_REPORT]\n");
			validParaLen = 1;
			if(getParaLen == validParaLen)
			{
				reportType = pBtReq->pParamStart[1];
				DBG_8723A("[MPT], BT_GGET_REPORT reportType=0x%x!!\n", reportType);
		...
		default:
			{
				DBG_8723A("[MPT], Error!! getType=%d, out of range\n", getType);
				pBtRsp->status = BT_STATUS_PARAMETER_OUT_OF_RANGE_U;
				return paraLen;
			}
			break;
	...
	if(getParaLen != validParaLen)
	{
		DBG_8723A("[MPT], Error!! wrong parameter length=%d for BT_GET_GEN_CMD cmd id=0x%x, paraLen should=0x%x\n",
	...
	if(BT_GGET_REG == getType)
	{
		...
		pu2Tmp = (pu2Byte)&pExtC2h->buf[0];
		regValue = *pu2Tmp;
		DBG_8723A("[MPT], read reg regType=0x%x, regAddr=0x%x, regValue=0x%x\n",
	...
	else if(BT_GGET_STATUS == getType)
	{
		...
		DBG_8723A("[MPT], read bt status, testMode=0x%x, testStatus=0x%x\n",
	...
	else if(BT_GGET_REPORT == getType)
	{
		switch(reportType)
		{
			case BT_REPORT_RX_PACKET_CNT:
				{
					DBG_8723A("[MPT], [Rx Packet Counts]\n");
			...
			case BT_REPORT_CFO_HDR_QUALITY:
				{
					DBG_8723A("[MPT], [CFO & Header Quality]\n");
			...
			case BT_REPORT_CONNECT_TARGET_BD_ADDR:
				{
					DBG_8723A("[MPT], [Connected Target BD ADDR]\n");
					...
					DBG_8723A("[MPT], Connected Target BDAddr:%s", bdAddr);
					for(i=0; i<6; i++)
					{
	...
	)
{
	...
	DBG_8723A("[MPT], setType=%d, setParaLen=%d\n", setType, setParaLen);

	/*  check parameter first */
	switch(setType)
	{
		case BT_GSET_REG:
			DBG_8723A ("[MPT], [BT_GSET_REG]\n");
			validParaLen = 9;
			if(setParaLen == validParaLen)
			{
				...
				regValue = *pu4Tmp;
				DBG_8723A("[MPT], BT_GSET_REG regType=0x%x, regAddr=0x%x, regValue=0x%x!!\n",
					regType, regAddr, regValue);
				if(regType >= BT_REG_MAX)
				{
					pBtRsp->status = (btOpcode<<8)| BT_STATUS_PARAMETER_OUT_OF_RANGE_U;
			...
			break;
		case BT_GSET_RESET:
			DBG_8723A("[MPT], [BT_GSET_RESET]\n");
			validParaLen = 0;
			break;
		case BT_GSET_TARGET_BD_ADDR:
			DBG_8723A("[MPT], [BT_GSET_TARGET_BD_ADDR]\n");
			validParaLen = 6;
			if(setParaLen == validParaLen)
			{
				...
				{
					DBG_8723A("[MPT], Error!! targetBDAddr=all zero\n");
					pBtRsp->status = (btOpcode<<8)|BT_STATUS_PARAMETER_OUT_OF_RANGE_U;
					return paraLen;
				}
				...
				{
					DBG_8723A("[MPT], Error!! targetBDAddr=all 0xf\n");
					pBtRsp->status = (btOpcode<<8)|BT_STATUS_PARAMETER_OUT_OF_RANGE_U;
					return paraLen;
				}
				...
				DBG_8723A ("[MPT], target BDAddr:%x,%x,%x,%x,%x,%x\n",
							bdAddr[0],bdAddr[1],bdAddr[2],bdAddr[3],bdAddr[4],bdAddr[5]);
			}
			break;
		case BT_GSET_TX_PWR_FINETUNE:
			DBG_8723A("[MPT], [BT_GSET_TX_PWR_FINETUNE]\n");
			validParaLen = 1;
			if(setParaLen == validParaLen)
			{
				...
				DBG_8723A ("[MPT], calVal=%d\n", calVal);
			}
			break;
		case BT_SET_TRACKING_INTERVAL:
			DBG_8723A("[MPT], [BT_SET_TRACKING_INTERVAL] setParaLen =%d \n",setParaLen);
		...
		case BT_SET_THERMAL_METER:
			DBG_8723A("[MPT], [BT_SET_THERMAL_METER] setParaLen =%d \n",setParaLen);
			validParaLen = 1;
			if(setParaLen == validParaLen)
				calVal = pBtReq->pParamStart[1];
			break;
		case BT_ENABLE_CFO_TRACKING:
			DBG_8723A("[MPT], [BT_ENABLE_CFO_TRACKING] setParaLen =%d \n",setParaLen);
			validParaLen = 1;
			if(setParaLen == validParaLen)
				calVal = pBtReq->pParamStart[1];
			break;
		case BT_GSET_UPDATE_BT_PATCH:
			...
			{
				u1Byte i;
				DBG_8723A ("[MPT], write regs for load patch\n");
				...
				for(i=0; i<12; i++)
				rtw_msleep_os(100);
				DBG_8723A("[MPT], load BT FW Patch finished!!!\n");
			}
			break;
		default:
			{
				DBG_8723A ("[MPT], Error!! setType=%d, out of range\n", setType);
				pBtRsp->status = BT_STATUS_PARAMETER_OUT_OF_RANGE_U;
				return paraLen;
			}
			break;
	...
	if(setParaLen != validParaLen)
	{
		DBG_8723A("[MPT], Error!! wrong parameter length=%d for BT_SET_GEN_CMD cmd id=0x%x, paraLen should=0x%x\n",
	...
	)
{
	...
	{
		DBG_8723A ("[MPT], pTxRxPars->txrxChannel=0x%x \n", pTxRxPars->txrxChannel);
		DBG_8723A ("[MPT], pTxRxPars->txrxTxPktCnt=0x%8x \n", pTxRxPars->txrxTxPktCnt);
		DBG_8723A  ("[MPT], pTxRxPars->txrxTxPktInterval=0x%x \n", pTxRxPars->txrxTxPktInterval);
		DBG_8723A  ("[MPT], pTxRxPars->txrxPayloadType=0x%x \n", pTxRxPars->txrxPayloadType);
		DBG_8723A  ("[MPT], pTxRxPars->txrxPktType=0x%x \n", pTxRxPars->txrxPktType);
		DBG_8723A  ("[MPT], pTxRxPars->txrxPayloadLen=0x%x \n", pTxRxPars->txrxPayloadLen);
		DBG_8723A  ("[MPT], pTxRxPars->txrxPktHeader=0x%x \n", pTxRxPars->txrxPktHeader);
		DBG_8723A  ("[MPT], pTxRxPars->txrxWhitenCoeff=0x%x \n", pTxRxPars->txrxWhitenCoeff);
		...
		DBG_8723A  ("[MPT], pTxRxPars->txrxBdaddr: %s", &bdAddr[0]);
		DBG_8723A ("[MPT], pTxRxPars->txrxTxGainIndex=0x%x \n", pTxRxPars->txrxTxGainIndex);
	}
	else
	{
		DBG_8723A  ("[MPT], Error!! pBtReq->paraLength=%d, correct Len=%d\n", pBtReq->paraLength, lenTxRx);
		pBtRsp->status = BT_STATUS_PARAMETER_FORMAT_ERROR_U;
		return paraLen;
	}
	...
	{
		DBG_8723A  ("[MPT], Error!! pTxRxPars->txrxPktHeader=0x%x is out of range, (should be between 0x0~0x3ffff)\n", pTxRxPars->txrxPktHeader);
		pBtRsp->status = (btOpcode<<8)|BT_STATUS_PARAMETER_OUT_OF_RANGE_U;
		return paraLen;
	}
	else
	...
	btOpcode = BT_LO_OP_SET_PKT_TYPE_LEN;
	{
		...
		switch(pTxRxPars->txrxPktType)
		{
			...
			default:
				{
					DBG_8723A  ("[MPT], Error!! Unknown pTxRxPars->txrxPktType=0x%x\n", pTxRxPars->txrxPktType);
					pBtRsp->status = (btOpcode<<8)|BT_STATUS_PARAMETER_OUT_OF_RANGE_U;
					return paraLen;
				}
				break;
		...
		{
			DBG_8723A ("[MPT], Error!! pTxRxPars->txrxPayloadLen=0x%x, (should smaller than %d)\n",
	...
	{
		DBG_8723A  ("[MPT], Error!! pTxRxPars->txrxPayloadType=0x%x, (should be between 0~4)\n", pTxRxPars->txrxPayloadType);
		pBtRsp->status = (btOpcode<<8)|BT_STATUS_PARAMETER_OUT_OF_RANGE_U;
		return paraLen;
	}
	else
	...
	{
	DBG_8723A  ("[MPT], Error!! pTxRxPars->txrxTxPktInterval=0x%x, (should be between 0~15)\n", pTxRxPars->txrxTxPktInterval);
	pBtRsp->status = (btOpcode<<8)|BT_STATUS_PARAMETER_OUT_OF_RANGE_U;
		return paraLen;
	}
	else
	...
	{
		DBG_8723A ("[MPT], Error!! pTxRxPars->txrxChannel=0x%x, (should be between 0~78)\n", pTxRxPars->txrxChannel);
		DBG_8723A ("[MPT], Error!! pTxRxPars->txrxTxGainIndex=0x%x, (should be between 0~7)\n", pTxRxPars->txrxTxGainIndex);
		pBtRsp->status = (btOpcode<<8)|BT_STATUS_PARAMETER_OUT_OF_RANGE_U;
		return paraLen;
	}
	else
	...
	{
		DBG_8723A ("[MPT], Error!! pTxRxPars->txrxBdaddr=all zero\n");
		pBtRsp->status = (btOpcode<<8)|BT_STATUS_PARAMETER_OUT_OF_RANGE_U;
		return paraLen;
	}
	...
	{
		DBG_8723A ("[MPT], Error!! pTxRxPars->txrxBdaddr=all 0xf\n");
		pBtRsp->status = (btOpcode<<8)|BT_STATUS_PARAMETER_OUT_OF_RANGE_U;
		return paraLen;
	}
	...
	)
{
	...
	{
		DBG_8723A("[MPT], Error!! testCtrl=0x%x, (should be between smaller or equal to 0x%x)\n",
	...
	)
{
	...
	DBG_8723A("[MPT], mptbt_BtControlProcess()=========>\n");

	DBG_8723A("[MPT], input opCodeVer=%d\n", pBtReq->opCodeVer);
	DBG_8723A("[MPT], input OpCode=%d\n", pBtReq->OpCode);
	DBG_8723A("[MPT], paraLength=%d \n", pBtReq->paraLength);
	if(pBtReq->paraLength)
	{
		/* DBG_8723A("[MPT], parameters(hex):0x%x %d \n",&pBtReq->pParamStart[0], pBtReq->paraLength); */
	}
