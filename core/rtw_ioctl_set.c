...
_func_enter_;
	...
	bdefaultkey=(wep->KeyIndex & 0x40000000) > 0 ? _FALSE : true;   /* for ??? */
	btransmitkey= (wep->KeyIndex & 0x80000000) > 0 ? true  : _FALSE;	/* for ??? */
...
_func_enter_;
	...
	else
	{
		...
		if((check_fwstate(&padapter->mlmepriv, WIFI_ADHOC_STATE)==true) && (!is_broadcast_ether_addr(key->BSSID))) {
