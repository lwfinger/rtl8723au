...
unsigned int OnProbeRsp(struct rtw_adapter *padapter,
			struct recv_frame *precv_frame)
{
	struct sta_info		*psta;
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
...
unsigned int OnBeacon(struct rtw_adapter *padapter,
		      struct recv_frame *precv_frame)
{
	int cam_idx;
	struct sta_info	*psta;
	...
	if (ether_addr_equal(hdr->addr3, get_my_bssid(&pmlmeinfo->network))) {
		...
		{
			if ((psta = rtw_get_stainfo(pstapriv, hdr->addr2)) !=
			    NULL) {
...
unsigned int OnAuthClient(struct rtw_adapter *padapter,
			  struct recv_frame *precv_frame)
{
...
unsigned int OnAssocReq(struct rtw_adapter *padapter, struct recv_frame *precv_frame)
{
	...
	pstat = rtw_get_stainfo(pstapriv, hdr->addr2);
	if (!pstat) {
...
#ifdef CONFIG_8723AU_AP_MODE
	issue_deauth(padapter, hdr->addr2, status);
#endif
...
unsigned int OnDeAuth(struct rtw_adapter *padapter,
		      struct recv_frame *precv_frame)
{
...
#endif
	{
		...
		receive_disconnect(padapter, hdr->addr3, reason);
	}
...
unsigned int OnAction_back(struct rtw_adapter *padapter, struct recv_frame *precv_frame)
{
	...
	if (category == WLAN_CATEGORY_BACK)/*  representing Block Ack */
	{
		...
		case WLAN_ACTION_ADDBA_REQ: /* ADDBA request */
			...
			process_addba_req(padapter, (u8*)&pmlmeinfo->ADDBA_req,
					  addr);
...
static void issue_p2p_GO_response(struct rtw_adapter *padapter, u8* raddr, u8* frame_body, uint len, u8 result)
{
	...
	*fctrl = 0;
	...
	pframe = rtw_set_fixed_ie(pframe, 1, &pwdinfo->negotiation_dialog_token,
				  &pattrib->pktlen);
	...
	if (wdev_to_priv(padapter->rtw_wdev)->p2p_enabled) {
		...
		switch (frame_body[ 6 ])/* OUI Subtype */
		{
			case P2P_GO_NEGO_REQ:
			{
				...
				if (ether_addr_equal(pwdinfo->rx_prov_disc_info.peerDevAddr, empty_addr))
			...
			case P2P_INVIT_REQ:
			{
				...
				if ((p2p_ie = rtw_get_p2p_ie(frame_body + _PUBLIC_ACTION_IE_OFFSET_, len - _PUBLIC_ACTION_IE_OFFSET_, NULL, &p2p_ielen)))
				{
					...
					issue_p2p_invitation_response(padapter, hdr->addr2, pwdinfo->inviteresp_info.token, status_code);
				}
				break;
			}
			case P2P_INVIT_RESP:
...
static unsigned int
on_action_public_default(struct recv_frame *precv_frame, u8 action)
{
	unsigned int ret = _FAIL;
	struct sk_buff *skb = precv_frame->pkt;
	u8 *pframe = skb->data;
...
unsigned int on_action_public(struct rtw_adapter *padapter,
			      struct recv_frame *precv_frame)
{
	unsigned int ret = _FAIL;
	struct sk_buff *skb = precv_frame->pkt;
...
unsigned int OnAction_p2p(struct rtw_adapter *padapter,
			  struct recv_frame *precv_frame)
{
#ifdef CONFIG_8723AU_P2P
	u8 *frame_body;
...
void site_survey(struct rtw_adapter *padapter)
{
	...
	if (0) {
		DBG_8723A(FUNC_ADPT_FMT" ch:%u (cnt:%u, idx:%d) at %dms, %c%c%c\n",
			  ...
			  ScanType?'A':'P',
			  pmlmeext->sitesurvey_res.scan_mode?'A':'P',
...
u8 collect_bss_info(struct rtw_adapter *padapter, struct recv_frame *precv_frame, struct wlan_bssid_ex *bssid)
{
	int	i;
	u32	len;
	u8	*p;
...
static u8 chk_ap_is_alive(struct rtw_adapter *padapter, struct sta_info *psta)
{
	...
	DBG_8723A(FUNC_ADPT_FMT" tx_pkts:%llu, link_count:%u\n",
		FUNC_ADPT_ARG(padapter), padapter->xmitpriv.tx_pkts,
