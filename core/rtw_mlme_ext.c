	...
	if (_rtw_memcmp(GetAddr3Ptr(pframe), get_my_bssid(&pmlmeinfo->network), ETH_ALEN)) {
		...
		if (((pmlmeinfo->state&0x03) == WIFI_FW_STATION_STATE) && (pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS)) {
			...
			if (psta != NULL) {
				...
				/* get supported rate */
				if (update_sta_support_rate(padapter, (pframe + WLAN_HDR_A3_LEN +
							    _BEACON_IE_OFFSET_),
							    (len - WLAN_HDR_A3_LEN -
...
unsigned int OnAssocRsp(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	/* AID */
	pmlmeinfo->aid = (int)(le16_to_cpu(*(unsigned short *)(pframe + WLAN_HDR_A3_LEN + 4))&0x3fff);
	res = pmlmeinfo->aid;
		...
		if (cpu_to_be32(*((u32 *)(frame_body + 2))) == P2POUI) {
...
#endif /* CONFIG_IOCTL_CFG80211 */
			{
				...
				case P2P_INVIT_REQ:
					...
					p2p_ie = rtw_get_p2p_ie(frame_body +
					...
					if (p2p_ie) {
						...
						if (attr_contentlen) {
							...
							if (invitation_flag & P2P_INVITATION_FLAGS_PERSISTENT) {
								...
								rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_GROUP_ID,
											 (u8 *)&group_id, &attr_contentlen);
								if (attr_contentlen) {
									if (_rtw_memcmp(group_id.go_device_addr, myid(&padapter->eeprompriv), ETH_ALEN)) {
										/* The p2p device sending this p2p invitation
										 * request wants this Wi-Fi device to be the persistent GO.
										 */
										...
										/* The p2p device sending this p2p invitation
										 * request wants to be the persistent GO.
										 */
										if (is_matched_in_profilelist(pwdinfo->p2p_peer_interface_addr,
													      &pwdinfo->profileinfo[0])) {
											...
											#ifdef CONFIG_INTEL_WIDI
											memcpy(pwdinfo->p2p_peer_device_addr,
											       group_id.go_device_addr, ETH_ALEN);
											rtw_p2p_set_role(pwdinfo, P2P_ROLE_CLIENT);
											#endif /* CONFIG_INTEL_WIDI */
...
void issue_asocrsp(_adapter *padapter, unsigned short status, struct sta_info *pstat, int pkt_type)
{
	...
	*(fctrl) = 0;
	...
	if ((pstat->flags & WLAN_STA_WME) && (pmlmepriv->qospriv.qos_option)) {
		...
		for (pbuf = ie + _BEACON_IE_OFFSET_; true; pbuf += (ie_len + 2)) {
		...
		if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE) && !rtw_p2p_chk_state(pwdinfo, P2P_STATE_IDLE)) {
			...
			*(u16 *)(p2pie + p2pielen) = cpu_to_le16(0x000D);
			...
			pframe = rtw_set_ie(pframe, _VENDOR_SPECIFIC_IE_,
					    p2pielen, (unsigned char *)p2pie, &pattrib->pktlen);
#ifdef CONFIG_WFD
...
static void process_80211d(PADAPTER padapter, WLAN_BSSID_EX *bssid)
{
	...
	if (pregistrypriv->enable80211d &&
		...
		while ((k < MAX_CHANNEL_NUM) && (chplan_new[k].ChannelNum != 0)) {
			DBG_871X("%02d(%c),", chplan_new[k].ChannelNum,
				 chplan_new[k].ScanType == SCAN_PASSIVE ? 'p' : 'c');
			k++;
		}
		DBG_871X("}\n");
#endif
	...
	if (is_client_associated_to_ap(padapter)) {
		...
		if (psta != NULL) {
			...
			DBG_871X("%s rx:"STA_PKTS_FMT", BI:%u, retry:%u\n",
				 __func__, STA_RX_PKTS_ARG(psta),
				 pmlmeinfo->bcn_interval, pmlmeext->retry);
			#endif
			...
			if (sta_last_rx_pkts(psta) == sta_rx_pkts(psta)) {
				...
				#endif
				{
					...
					DBG_871X_LEVEL(_drv_always_,
						       "no beacon for a long time, disconnect or roaming\n");
