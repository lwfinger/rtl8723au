...
static void init_channel_list(struct rtw_adapter *padapter, RT_CHANNEL_INFO *channel_set,
							  u8 chanset_size,
		...
		for(index=0;index<RTW_ChannelPlan5G[Index5G].Len;index++)
		{
			...
			if ( RTW_ChannelPlan5G[Index5G].Channel[index] <= 48
				...
				if(RT_CHANNEL_DOMAIN_WORLD_WIDE_5G == ChannelPlan)/* passive scan for all 5G channels */
					...
					channel_set[chanset_size].ScanType = SCAN_ACTIVE;
				DBG_8723A("%s(): channel_set[%d].ChannelNum = %d\n", __FUNCTION__, chanset_size, channel_set[chanset_size].ChannelNum);
				chanset_size++;
			}
#endif /* CONFIG_DFS */
		}
...
void mgt_dispatcher(struct rtw_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if((index << 4)==WIFI_ACTION){
		...
		if(*(pframe+24)==0x04 && *(pframe+25)==TDLS_DISCOVERY_RESPONSE){
			DBG_8723A("recv tdls discovery response frame\n");
	...
	if (psta != NULL)
	{
		if (GetRetry(pframe))
		{
			if (precv_frame->u.hdr.attrib.seq_num == psta->RxMgmtFrameSeqNum)
			{
				...
				DBG_8723A("Drop duplicate management frame with seq_num = %d.\n", precv_frame->u.hdr.attrib.seq_num);
				return;
			}
		}
...
#ifdef CONFIG_P2P
static u32 p2p_listen_state_process(struct rtw_adapter *padapter, unsigned char *da)
{
	bool response = _TRUE;
...
unsigned int OnProbeRsp(struct rtw_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if(rtw_p2p_chk_state(pwdinfo, P2P_STATE_TX_PROVISION_DIS_REQ))
	{
		...
		{
			if (!memcmp(pwdinfo->tx_prov_disc_info.peerIFAddr,
	...
	if (!memcmp(GetAddr3Ptr(pframe), get_my_bssid(&pmlmeinfo->network),
		...
		if(((pmlmeinfo->state&0x03) == WIFI_FW_STATION_STATE) && (pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS))
		{
			if ((psta = rtw_get_stainfo(pstapriv, GetAddr2Ptr(pframe))) != NULL)
			{
				...
				if (!ret) {
						DBG_8723A_LEVEL(_drv_always_, "ap has changed, disconnect now\n ");
...
unsigned int OnAuth(struct rtw_adapter *padapter, union recv_frame *precv_frame)
{
	...
	{
		DBG_8723A("auth rejected due to bad alg [alg=%d, auth_mib=%d] %02X%02X%02X%02X%02X%02X\n",
	...
	if (pstat == NULL)
	{
		...
		if (pstat == NULL)
		{
			DBG_8723A(" Exceed the upper limit of supported clients...\n");
			status = _STATS_UNABLE_HANDLE_STA_;
			goto auth_fail;
		}
	...
	{
		DBG_8723A("(1)auth rejected because out of seq [rx_seq=%d, exp_seq=%d]!\n",
	...
	if (algorithm==0 && (auth_mode == 0 || auth_mode == 2))
	{
		...
		{
			DBG_8723A("(2)auth rejected because out of seq [rx_seq=%d, exp_seq=%d]!\n",
				seq, pstat->auth_seq+1);
			status = _STATS_OUT_OF_AUTH_SEQ_;
			goto auth_fail;
		}
	...
	else /*  shared system or auto authentication */
	{
		...
		else if (seq == 3)
		{
			...
			if((p==NULL) || (ie_len<=0))
			{
				DBG_8723A("auth rejected because challenge failure!(1)\n");
			...
			{
				DBG_8723A("auth rejected because challenge failure!\n");
				status = _STATS_CHALLENGE_FAIL_;
				goto auth_fail;
			}
		}
		...
		{
			DBG_8723A("(3)auth rejected because out of seq [rx_seq=%d, exp_seq=%d]!\n",
				seq, pstat->auth_seq+1);
			status = _STATS_OUT_OF_AUTH_SEQ_;
			goto auth_fail;
		}
...
unsigned int OnDeAuth(struct rtw_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if(check_fwstate(pmlmepriv, WIFI_AP_STATE) == _TRUE)
	{
		...
		DBG_8723A_LEVEL(_drv_always_, "ap recv deauth reason code(%d) sta:%pM\n",
...
#endif
	{
		DBG_8723A_LEVEL(_drv_always_, "sta recv deauth reason code(%d) sta:%pM\n",
...
unsigned int OnDisassoc(struct rtw_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if(check_fwstate(pmlmepriv, WIFI_AP_STATE) == _TRUE)
	{
		...
		DBG_8723A_LEVEL(_drv_always_, "ap recv disassoc reason code(%d) sta:%pM\n",
				reason, GetAddr2Ptr(pframe));
...
static unsigned int on_action_spct_ch_switch(struct rtw_adapter *padapter, struct sta_info *psta, u8 *ies, uint ies_len)
{
	...
	if ((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE) {
		...
		DBG_8723A(FUNC_NDEV_FMT" from "MAC_FMT"\n",
		...
		for_each_ie(ie, ies, ies_len) {
			if (ie->id == WLAN_EID_CHANNEL_SWITCH) {
				...
				DBG_8723A("ch_switch_mode:%d, ch:%d, ch_switch_cnt:%d\n",
...
static u8 is_matched_in_profilelist( u8* peermacaddr, struct profile_info* profileinfo )
{
	...
	{
	       DBG_8723A( "[%s] profileinfo_mac = %.2X %.2X %.2X %.2X %.2X %.2X\n", __FUNCTION__,
...
static int _issue_probereq_p2p(struct rtw_adapter *padapter, u8 *da, int wait_ack)
{
	int ret = _FAIL;
	struct xmit_frame		*pmgntframe;
	struct pkt_attrib		*pattrib;
...
int issue_probereq_p2p_ex(struct rtw_adapter *adapter, u8 *da, int try_cnt, int wait_ms)
{
	...
	if (try_cnt && wait_ms) {
		if (da)
			DBG_8723A(FUNC_ADPT_FMT" to "MAC_FMT", ch:%u%s, %d/%d in %u ms\n",
...
static s32 rtw_action_public_decache(union recv_frame *recv_frame, s32 token)
{
	...
	if (GetRetry(frame)) {
		if (token >= 0) {
			...
			{
				DBG_8723A(FUNC_ADPT_FMT" seq_ctrl=0x%x, rxseq=0x%x, token:%d\n",
					FUNC_ADPT_ARG(adapter), seq_ctrl, mlmeext->action_public_rxseq, token);
				return _FAIL;
			}
		} else {
			if (seq_ctrl == mlmeext->action_public_rxseq) {
				DBG_8723A(FUNC_ADPT_FMT" seq_ctrl=0x%x, rxseq=0x%x\n",
...
unsigned int OnAction_p2p(struct rtw_adapter *padapter, union recv_frame *precv_frame)
{
	...
	/* check RA matches or not */
	if (memcmp(myid(&(padapter->eeprompriv)),
		   GetAddr1Ptr(pframe), ETH_ALEN))/* for if1, sta/ap mode */
		return _SUCCESS;
	...
	if( (pmlmeinfo->state&0x03) == WIFI_FW_AP_STATE)
	{
		...
		/* retrieve SSID IE from cur_network->Ssid */
		{
			...
			if (ssid_ie &&  cur_network->Ssid.SsidLength) {
				...
				DBG_8723A_LEVEL(_drv_warning_, FUNC_ADPT_FMT" remainder_ielen > MAX_IE_SZ\n", FUNC_ADPT_ARG(padapter));
...
static int _issue_probereq(struct rtw_adapter *padapter, NDIS_802_11_SSID *pssid, u8 *da, int wait_ack)
{
	int ret = _FAIL;
	struct xmit_frame		*pmgntframe;
	struct pkt_attrib		*pattrib;
	...
	*(fctrl) = 0;
	...
	for (i = 0; i < NDIS_802_11_LENGTH_RATES_EX; i++) {
		...
		if (j == sta_bssrate_len) {
			...
			DBG_8723A("%s(): the rate[%d]=%02X is not supported by STA!\n",__FUNCTION__, i, pmlmeinfo->network.SupportedRates[i]);
		} else {
...
void site_survey(struct rtw_adapter *padapter)
{
	...
	if (0){
		DBG_8723A(FUNC_ADPT_FMT" ch:%u (cnt:%u,idx:%d) at %dms, %c%c%c\n"
		, FUNC_ADPT_ARG(padapter)
		, survey_channel
	...
	else
	{
...
#endif /* CONFIG_P2P */
		{
			...
			if (is_client_associated_to_ap(padapter) == _TRUE)
			{
				...
				{
					DBG_8723A("adapter is surveydone(buddy_adapter is linked), issue nulldata(pwrbit=0)\n");
			...
			check_buddy_fwstate(padapter, _FW_LINKED))
		{
			...
			DBG_8723A("survey done, current CH=%d, BW=%d, offset=%d\n", cur_channel, cur_bwmode, cur_ch_offset);

			DBG_8723A("restart pbuddy_adapter's beacon\n");
...
u8 collect_bss_info(struct rtw_adapter *padapter, union recv_frame *precv_frame, WLAN_BSSID_EX *bssid)
{
	...
	if(strcmp(bssid->Ssid.Ssid, DBG_RX_SIGNAL_DISPLAY_SSID_MONITORED) == 0) {
		DBG_8723A("Receiving %s("MAC_FMT", DSConfig:%u) from ch%u with ss:%3u, sq:%3u, RawRSSI:%3ld\n"
...
void start_create_ibss(struct rtw_adapter* padapter)
{
	...
	{
		DBG_8723A("start_create_ibss, invalid cap:%x\n", caps);
		return;
	}
}
...
void start_clnt_join(struct rtw_adapter* padapter)
{
	...
	if (caps&cap_ESS)
	{
	       ...
	       {
		   DBG_8723A("dc_handle_join_request fail !!!\n");
		   return;
	        }
	        /* switch channel */
	       set_channel_bwmode(padapter, pmlmeext->cur_channel, pmlmeext->cur_ch_offset, pmlmeext->cur_bwmode);
	...
	else if (caps&cap_IBSS) /* adhoc client */
	{
		...
		if(dc_handle_join_request(padapter) == _FAIL)
		{
			DBG_8723A("dc_handle_join_request for Ad-hoc fail !!!\n");
			return;
		}
#endif
...
static void process_80211d(struct rtw_adapter *padapter, WLAN_BSSID_EX *bssid)
{
		...
		(!pmlmeext->update_channel_plan_by_ap_done))
	{
		...
		{
			DBG_8723A("%02d,", chplan_ap.Channel[i]);
			i++;
		}
		DBG_8723A("}\n");
#endif
		...
		{
			DBG_8723A("%02d(%c),", chplan_sta[i].ChannelNum, chplan_sta[i].ScanType==SCAN_PASSIVE?'p':'a');
			i++;
		}
		DBG_8723A("}\n");
#endif
...
void report_del_sta_event(struct rtw_adapter *padapter, unsigned char* MacAddr, unsigned short reason)
{
	...
	DBG_8723A("report_del_sta_event: delete STA, mac_id=%d\n", mac_id);
...
void report_add_sta_event(struct rtw_adapter *padapter, unsigned char* MacAddr, int cam_idx)
{
	...
	DBG_8723A("report_add_sta_event: add STA\n");
...
static u8 chk_ap_is_alive(struct rtw_adapter *padapter, struct sta_info *psta)
{
	...
	DBG_8723A(FUNC_ADPT_FMT" rx:"STA_PKTS_FMT", beacon:%llu, probersp_to_self:%llu"
	...
	DBG_8723A(FUNC_ADPT_FMT" tx_pkts:%llu, link_count:%u\n", FUNC_ADPT_ARG(padapter)
		...
		if ((psta = rtw_get_stainfo(pstapriv, pmlmeinfo->network.MacAddress)) != NULL)
		{
			...
			#endif /* CONFIG_ACTIVE_KEEP_ALIVE_CHECK */
			{
				if (rx_chk != _SUCCESS) {
					if (pmlmeext->retry == 0) {
						#ifdef DBG_EXPIRATION_CHK
						DBG_8723A("issue_probereq to trigger probersp, retry=%d\n", pmlmeext->retry);
						#endif
...
u8 sitesurvey_cmd_hdl(struct rtw_adapter *padapter, u8 *pbuf)
{
	...
	if (pmlmeext->sitesurvey_res.state == SCAN_DISABLE)
	{
		...
		if (is_client_associated_to_ap(padapter) == _TRUE)
		{
			...
			if(is_client_associated_to_ap(padapter->pbuddy_adapter) == _TRUE)
			{
				DBG_8723A("adapter is scanning(buddy_adapter is linked), issue nulldata(pwrbit=1)\n");
...
*/
	...
	if((pmlmeinfo->state&0x03) == WIFI_FW_AP_STATE)
	{
		...
		if(psta)
		{
			...
			DBG_8723A("r871x_set_stakey_hdl(): enc_algorithm=%d\n", pparm->algorithm);

			if((psta->mac_id<1) || (psta->mac_id>(NUM_STA-4)))
			{
				DBG_8723A("r871x_set_stakey_hdl():set_stakey failed, mac_id(aid)=%d\n", psta->mac_id);
				return H2C_REJECTED;
			}
		...
		{
			DBG_8723A("r871x_set_stakey_hdl(): sta has been free\n");
			return H2C_REJECTED;
		}
...
static void dc_change_band(struct rtw_adapter *padapter, WLAN_BSSID_EX *pnetwork)
{
	...
	else
	{
		...
		DBG_8723A("%s(): change to 2.4G Band\n",__FUNCTION__);
...
void dc_handle_join_done(struct rtw_adapter *padapter, u8 join_res)
{
		...
		padapter->DualMacConcurrent == _TRUE)
	{
				...
				check_fwstate(pbuddy_mlmepriv, _FW_LINKED))
		{
			...
			DBG_8723A("after join, current adapter, CH=%d, BW=%d, offset=%d\n", pmlmeext->cur_channel, pmlmeext->cur_bwmode, pmlmeext->cur_ch_offset);
			...
			DBG_8723A("after join, another adapter, CH=%d, BW=%d, offset=%d\n", pbuddy_mlmeext->cur_channel, pbuddy_mlmeext->cur_bwmode, pbuddy_mlmeext->cur_ch_offset);

			if(change_band == _TRUE)
				dc_change_band(pbuddy_adapter, pbuddy_network_mlmeext);

			DBG_8723A("update pbuddy_adapter's beacon\n");
...
void dc_set_ap_channel_bandwidth(struct rtw_adapter *padapter, u8 channel, u8 channel_offset, u8 bwmode)
{
	...
	DBG_8723A("dualmac_concurrent_ap_set_channel_bwmode ==>\n");
		...
		padapter->DualMacConcurrent == _TRUE)
	{
		...
		else if(check_fwstate(pbuddy_mlmepriv, _FW_LINKED)==_TRUE)
		{
			...
			DBG_8723A("Another iface is at linked state, sync cur_channel/cur_bwmode/cur_ch_offset\n");
			DBG_8723A("Another adapter, CH=%d, BW=%d, offset=%d\n", pbuddy_mlmeext->cur_channel, pbuddy_mlmeext->cur_bwmode, pbuddy_mlmeext->cur_ch_offset);
			DBG_8723A("Current adapter, CH=%d, BW=%d, offset=%d\n", cur_channel, cur_bwmode, cur_ch_offset);
...
void dc_resume_xmit(struct rtw_adapter *padapter)
{
	...
	if(pbuddy_adapter != NULL &&
		padapter->DualMacConcurrent == _TRUE)
	{
		DBG_8723A("dc_resume_xmit,  resume pbuddy_adapter Tx\n");
...
u8	dc_check_xmit(struct rtw_adapter *padapter)
{
		...
		padapter->DualMacConcurrent == _TRUE)
	{
		...
		{
			DBG_8723A("dc_check_xmit  pbuddy_adapter is under survey or under linking\n");
			return _FALSE;
		}
	}
...
int concurrent_chk_start_clnt_join(struct rtw_adapter *padapter)
{
		...
		check_fwstate(pbuddy_mlmepriv, WIFI_STATION_STATE) == _TRUE) /* for Client Mode/p2p client */
	{
		...
		if(pmlmeext->cur_channel != pbuddy_mlmeext->cur_channel)
		{
			DBG_8723A("start_clnt_join(ch=%d), but channel mismatch with buddy(ch=%d) interface\n",
		...
		if((pmlmeext->cur_bwmode == HT_CHANNEL_WIDTH_40) &&
			...
			(pmlmeext->cur_ch_offset != pbuddy_mlmeext->cur_ch_offset))
		{
			DBG_8723A("start_clnt_join(bwmode=%d, ch_offset=%d), but bwmode & ch_offset mismatch with buddy(bwmode=%d, ch_offset=%d) interface\n",
...
void concurrent_chk_joinbss_done(struct rtw_adapter *padapter, int join_res)
{
			...
			check_fwstate(pbuddy_mlmepriv, _FW_LINKED))
	{
		...
		DBG_8723A("after join,primary adapter, CH=%d, BW=%d, offset=%d\n"
			, pmlmeext->cur_channel, pmlmeext->cur_bwmode, pmlmeext->cur_ch_offset);
		...
		DBG_8723A("after join, second adapter, CH=%d, BW=%d, offset=%d\n", pbuddy_mlmeext->cur_channel, pbuddy_mlmeext->cur_bwmode, pbuddy_mlmeext->cur_ch_offset);
...
u8 set_csa_hdl(struct rtw_adapter *padapter, unsigned char *pbuf)
{
	...
	DBG_8723A("DFS detected! Swiching channel to %d!\n", new_ch_no);
	...
	if ( ((new_ch_no >= 52) && (new_ch_no <= 64)) ||((new_ch_no >= 100) && (new_ch_no <= 140)) ) {
		DBG_8723A("Switched to DFS band (ch %02x) again!!\n", new_ch_no);
	}
...
u8 tdls_hdl(struct rtw_adapter *padapter, unsigned char *pbuf)
{
	...
	DBG_8723A("[%s] option:%d\n", __FUNCTION__, option);

	switch(option){
		case TDLS_WRCR:
			/* As long as TDLS handshake success, we should set RCR_CBSSID_DATA bit to 0 */
			/* such we can receive all kinds of data frames. */
			rtw_hal_set_hwreg(padapter, HW_VAR_TDLS_WRCR, NULL);
			DBG_8723A("TDLS with "MAC_FMT"\n", MAC_ARG(ptdls_sta->hwaddr));
		...
		case TDLS_CS_OFF:
			...
			DBG_8723A("go back to base channel\n ");
			issue_nulldata(padapter, NULL, 0, 0, 0);
			break;
		case TDLS_INIT_CH_SEN:
			rtw_hal_set_hwreg(padapter, HW_VAR_TDLS_INIT_CH_SEN, NULL);
			pmlmeext->sitesurvey_res.channel_idx = 0;
		...
		case TDLS_DONE_CH_SEN:
			...
			if(survey_channel){
				...
				rtw_hal_set_hwreg(padapter, HW_VAR_TDLS_DONE_CH_SEN, NULL);
				...
				if(ptdlsinfo->ch_sensing==1){
					...
					DBG_8723A("TDLS channel sensing done, candidate channel: %02x\n", ptdlsinfo->candidate_ch);
				...
				if(ptdls_sta->tdls_sta_state & TDLS_PEER_SLEEP_STATE){
					...
					DBG_8723A("issue tdls ch switch req\n");
				}
			}
			break;
		case TDLS_OFF_CH:
			...
			DBG_8723A("change channel to tar ch:%02x\n", ptdls_sta->off_ch);
		...
		case TDLS_P_OFF_CH:
			SelectChannel(padapter, pmlmeext->cur_channel);
			issue_nulldata(padapter, NULL, 0, 0, 0);
			DBG_8723A("change channel to base ch:%02x\n", pmlmeext->cur_channel);
		...
		case TDLS_P_BASE_CH:
			...
			SelectChannel(padapter, ptdls_sta->off_ch);
			DBG_8723A("change channel to off ch:%02x\n", ptdls_sta->off_ch);
			ptdls_sta->tdls_sta_state |= TDLS_AT_OFF_CH_STATE;
			...
			break;
		case TDLS_RS_RCR:
			rtw_hal_set_hwreg(padapter, HW_VAR_TDLS_RS_RCR, NULL);
			DBG_8723A("wirte REG_RCR, set bit6 on\n");
			break;
		case TDLS_CKALV_PH1:
