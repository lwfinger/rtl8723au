...
void rtw_tdls_process_wfd_ie(struct tdls_info *ptdlsinfo, u8 *ptr, u8 length)
{
	...
	while( wfd_offset )
	{
		...
		if ( attr_contentlen )
		{
			...
			DBG_8723A( "[%s] Peer IP = %02u.%02u.%02u.%02u \n", __FUNCTION__,
...
void issue_tdls_teardown(struct rtw_adapter *padapter, u8 *mac_addr)
{
	...
	ptdls_sta = rtw_get_stainfo(pstapriv, mac_addr);
	if(ptdls_sta==NULL){
		DBG_8723A("issue tdls teardown unsuccessful\n");
		return;
	}else{
...
int On_TDLS_Dis_Rsp(struct rtw_adapter *adapter, union recv_frame *precv_frame)
{
	...
	else
	{
		...
		{
			DBG_8723A("pattrib->RxPWDBAll=%d, pdmpriv->UndecoratedSmoothedPWDB=%d\n", pattrib->RxPWDBAll, UndecoratedSmoothedPWDB);
...
int On_TDLS_Setup_Req(struct rtw_adapter *adapter, union recv_frame *precv_frame)
{
	...
	if(ptdls_sta==NULL){
		...
		if(ptdls_sta->tdls_sta_state & TDLS_LINKED_STATE){
			...
			DBG_8723A("re-setup a direct link\n");
		}
		/* already receiving TDLS setup request */
		else if(ptdls_sta->tdls_sta_state & TDLS_INITIATOR_STATE){
			DBG_8723A("receive duplicated TDLS setup request frame in handshaking\n");
			goto exit;
		}
		...
		/* following is to check out MAC_addr */
			...
			DBG_8723A("receive setup_req after sending setup_req\n");
			for (i=0;i<6;i++){
				if(*(pmyid+i)==*(psa+i)){
				}
...
int On_TDLS_Setup_Rsp(struct rtw_adapter *adapter, union recv_frame *precv_frame)
{
	...
	if(stat_code!=0)
	{
		DBG_8723A( "[%s] status_code = %d, free_tdls_sta\n", __FUNCTION__, stat_code );
		free_tdls_sta(adapter, ptdls_sta);
		return _FAIL;
	}
	...
	for(j=FIXED_IE; j<parsing_length;)
	{
		...
		switch (pIE->ElementID)
		{
			...
			case _RSN_IE_2_:
				...
				for(k=0;k<pairwise_count;k++){
					if (!memcmp(ppairwise_cipher + 4*k,
	...
	DBG_8723A("issue_tdls_setup_cfm\n");
...
int On_TDLS_Peer_Traffic_Rsp(struct rtw_adapter *adapter, union recv_frame *precv_frame)
{
	...
	if(ptdls_sta->tdls_sta_state & TDLS_LINKED_STATE){
		if(wmmps_ac && state)
		{
			...
			if(ptdls_sta->sleepq_len==0)
			{
				DBG_8723A("no buffered packets for tdls to xmit\n");
...
int On_TDLS_Ch_Switch_Rsp(struct rtw_adapter *adapter, union recv_frame *precv_frame)
{
	...
	if(ptdls_sta->tdls_sta_state & TDLS_CH_SWITCH_ON_STATE ){
		if(pmlmeext->cur_channel==ptdls_sta->off_ch){
			...
			DBG_8723A("receive unsolicited channel switch response \n");
			rtw_tdls_cmd(adapter, ptdls_sta->hwaddr, TDLS_CS_OFF);
		}
		return _FAIL;
	}
...
void rtw_build_tdls_setup_rsp_ies(struct rtw_adapter * padapter, struct xmit_frame * pxmitframe, u8 *pframe)
{
	...
	{
		DBG_8723A("ptdls_sta->stat_code:%04x \n", ptdls_sta->stat_code);
		return;
	}
...
void _tdls_handshake_timer_hdl(void *FunctionContext)
{
	...
	if(ptdls_sta != NULL)
	{
		if( !(ptdls_sta->tdls_sta_state & TDLS_LINKED_STATE) )
		{
			DBG_8723A("tdls handshake time out\n");
...
void _tdls_alive_timer_phase1_hdl(void *FunctionContext)
{
	...
	DBG_8723A("issue_tdls_dis_req to check alive\n");
...
void _tdls_alive_timer_phase2_hdl(void *FunctionContext)
{
	...
	{
		DBG_8723A("TDLS STA ALIVE, ptdls_sta->sta_stats.last_rx_pkts:%llu, ptdls_sta->sta_stats.rx_pkts:%llu\n",
	...
	{
		if( !(ptdls_sta->tdls_sta_state & TDLS_ALIVE_STATE) )
			DBG_8723A("TDLS STA TOO FAR\n");
		if( !(sta_last_rx_pkts(ptdls_sta) + 3 <= sta_rx_pkts(ptdls_sta)))
			DBG_8723A("TDLS LINK WITH LOW TRAFFIC, ptdls_sta->sta_stats.last_rx_pkts:%llu, ptdls_sta->sta_stats.rx_pkts:%llu\n",
				sta_last_rx_pkts(ptdls_sta), sta_rx_pkts(ptdls_sta));
