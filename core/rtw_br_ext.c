...
static int update_nd_link_layer_addr(unsigned char *data, int len, unsigned char *replace_mac)
{
	...
	if (icmphdr->icmp6_type == NDISC_ROUTER_SOLICITATION) {
		if (len >= 8) {
			...
			if (mac) {
				_DEBUG_INFO("Router Solicitation, replace MAC From: %pM To: %pM\n",
					    mac, replace_mac);
		...
		if (len >= 16) {
			...
			if (mac) {
				_DEBUG_INFO("Router Advertisement, replace MAC From: %pM To: %pM\n",
					    mac, replace_mac);
				memcpy(mac, replace_mac, 6);
				return 1;
			}
		...
		if (len >= 24) {
			...
			if (mac) {
				_DEBUG_INFO("Neighbor Solicitation, replace MAC From: %pM To: %pM\n",
					    mac, replace_mac);
				memcpy(mac, replace_mac, 6);
				return 1;
			}
		...
		if (len >= 24) {
			...
			if (mac) {
				_DEBUG_INFO("Neighbor Advertisement, replace MAC From: %pM To: %pM\n",
					    mac, replace_mac);
				memcpy(mac, replace_mac, 6);
				return 1;
			}
		...
		if (len >= 40) {
			...
			if (mac) {
				_DEBUG_INFO("Redirect,  replace MAC From: %pM To: %pM\n",
					    mac, replace_mac);
				memcpy(mac, replace_mac, 6);
				return 1;
			}
...
int nat25_db_handle(_adapter *priv, struct sk_buff *skb, int method)
{
	...
	/*---------------------------------------------------*/
		...
		case NAT25_CHECK:
			...
			return -1;
		case NAT25_INSERT:
			DEBUG_INFO("NAT25: Insert IP, SA =%4x:%4x:%4x:%4x:%4x:%4x:%4x:%4x, DA =%4x:%4x:%4x:%4x:%4x:%4x:%4x:%4x\n",
