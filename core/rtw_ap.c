...
void	expire_timeout_chk(_adapter *padapter)
{
	...
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE)
	{
		...
		if ( (psta->flags & WLAN_STA_HT) && (psta->htpriv.agg_enable_bitmap || psta->under_exist_checking) ) {
			...
			//DBG_8723A("asoc check by DELBA/ADDBA! (pstapriv->expire_to=%d s)(psta->expire_to=%d s), [%02x, %d]\n", pstapriv->expire_to*2, psta->expire_to*2, psta->htpriv.agg_enable_bitmap, psta->under_exist_checking);
		...
		if (psta->expire_to <= 0)
		{
			...
			if (psta->state & WIFI_SLEEP_STATE) {
				if (!(psta->state & WIFI_STA_ALIVE_CHK_STATE)) {
					...
					//DBG_8723A("alive chk, sta:" MAC_FMT " is at ps mode!\n", MAC_ARG(psta->hwaddr));
...
static void update_bmc_sta(_adapter *padapter)
{
	...
	if(psta)
	{
		...
		//DBG_8723A("Add id %d val %08x to ratr for bmc sta\n", psta->aid, tx_ra_bitmap);
		//ap mode
