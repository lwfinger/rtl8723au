...
_func_enter_;
	...
	while (p2p_ie)
	{
		...
		if (rtw_get_p2p_attr_content( p2p_ie, p2p_ielen, P2P_ATTR_NOA, noa_attr, &attr_contentlen))
		{
			...
			if ( (pwdinfo->p2p_ps_enable == false) ||
				(noa_index != pwdinfo->noa_index) )// if index change, driver should reconfigure related setting.
			{
				...
				else if ( pwdinfo->p2p_ps_enable == true)
				{
	...
	if (find_p2p == true)
	{
		if ( (pwdinfo->p2p_ps_enable == true) && (find_p2p_ps == false) )
		{
			p2p_ps_wk_cmd(padapter, P2P_PS_DISABLE, 1);
		}
	}
	...
	if (category == RTW_WLAN_CATEGORY_PUBLIC)
	{
		...
		else
		{
			DBG_871X("RTW_%s:ACTION_CATEGORY_PUBLIC: action=%d, OUI=0x%x, OUI_Subtype=%d, dialogToken=%d\n",
					(tx==true)?"TX":"RX", action, cpu_to_be32( *( ( u32* ) ( frame_body + 2 ) ) ), OUI_Subtype, dialogToken);
		}
...
void init_wifidirect_info( _adapter* padapter, enum P2P_ROLE role)
{
	...
	memcpy(pwdinfo->device_addr, myid(&(padapter->eeprompriv)), ETH_ALEN);
	memcpy(pwdinfo->interface_addr, myid(&(padapter->eeprompriv)), ETH_ALEN);
...
//	Use the OFDM rate in the P2P probe response frame. ( 6(B), 9(B), 12, 18, 24, 36, 48, 54 )
	...
	memcpy( pwdinfo->device_name, "Realtek DMP Device", 18 );
