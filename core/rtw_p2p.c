		...
		if ( rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_CH_LIST, ch_content, &ch_cnt) )
		{
			...
			if(rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO))
			{
												...
												ch_list_inclusioned, ch_num_inclusioned) )
				{
					...
					{
						DBG_8723A( "[%s] desired channel NOT Found!\n", __FUNCTION__ );
						result = P2P_STATUS_FAIL_NO_COMMON_CH;
						rtw_p2p_set_state(pwdinfo, P2P_STATE_GONEGO_FAIL);
						break;
					}
...
#ifdef CONFIG_DEBUG_CFG80211
		DBG_8723A("ACTION_CATEGORY_P2P: OUI=0x%x, OUI_Subtype=%d, dialogToken=%d\n",
...
int rtw_p2p_check_frames(struct rtw_adapter *padapter, const u8 *buf, u32 len, u8 tx)
{
	...
	if(category == RTW_WLAN_CATEGORY_PUBLIC)
	{
		...
		if (action == ACT_PUBLIC_VENDOR &&
			...
			#ifdef CONFIG_DEBUG_CFG80211
			DBG_8723A("ACTION_CATEGORY_PUBLIC: ACT_PUBLIC_VENDOR, OUI=0x%x, OUI_Subtype=%d, dialogToken=%d\n",
...
#endif
{
	...
	struct rtw_adapter *adapter = (struct rtw_adapter *)FunctionContext;
#else
	struct rtw_adapter *adapter = from_timer(adapter, t, wdinfo.reset_ch_sitesurvey);
#endif
...
#endif
{
	...
	struct rtw_adapter *adapter = (struct rtw_adapter *)FunctionContext;
#else
	struct rtw_adapter *adapter = from_timer(adapter, t, wdinfo.reset_ch_sitesurvey2);
#endif
	struct	wifidirect_info		*pwdinfo = &adapter->wdinfo;
...
#endif
{
	...
	struct rtw_adapter *adapter = (struct rtw_adapter *)FunctionContext;
#else
	struct rtw_adapter *adapter = from_timer(adapter, t, wdinfo.restore_p2p_state_timer);
#endif
	struct	wifidirect_info		*pwdinfo = &adapter->wdinfo;
...
#endif
{
	...
	struct rtw_adapter	*adapter = (struct rtw_adapter *) FunctionContext;
#else
	struct rtw_adapter *adapter = from_timer(adapter, t, wdinfo.pre_tx_scan_timer);
#endif
	struct	wifidirect_info				*pwdinfo = &adapter->wdinfo;
...
#endif
{
	...
	struct rtw_adapter *adapter = (struct rtw_adapter *)FunctionContext;
#else
	struct rtw_adapter *adapter = from_timer(adapter, t, wdinfo.find_phase_timer);
#endif
	struct	wifidirect_info		*pwdinfo = &adapter->wdinfo;
...
#endif
{
	...
	struct rtw_adapter *adapter = (struct rtw_adapter *)FunctionContext;
#else
	struct rtw_adapter *adapter = from_timer(adapter, t, wdinfo.ap_p2p_switch_timer);
#endif
	struct	wifidirect_info		*pwdinfo = &adapter->wdinfo;
#ifdef CONFIG_IOCTL_CFG80211
	struct rtw_wdev_priv *pwdev_priv = wdev_to_priv(adapter->rtw_wdev);
...
void dbg_rtw_p2p_set_state(struct wifidirect_info *wdinfo, enum P2P_STATE state, const char *caller, int line)
{
	if(!_rtw_p2p_chk_state(wdinfo, state)) {
		...
		DBG_8723A("[CONFIG_DBG_P2P]%s:%d set_state from %s to %s\n", caller, line
			, p2p_state_str[old_state], p2p_state_str[_rtw_p2p_state(wdinfo)]
		);
	} else {
		DBG_8723A("[CONFIG_DBG_P2P]%s:%d set_state to same state %s\n", caller, line
...
void dbg_rtw_p2p_set_pre_state(struct wifidirect_info *wdinfo, enum P2P_STATE state, const char *caller, int line)
{
	if(_rtw_p2p_pre_state(wdinfo) != state) {
		...
		DBG_8723A("[CONFIG_DBG_P2P]%s:%d set_pre_state from %s to %s\n", caller, line
			, p2p_state_str[old_state], p2p_state_str[_rtw_p2p_pre_state(wdinfo)]
		);
	} else {
		DBG_8723A("[CONFIG_DBG_P2P]%s:%d set_pre_state to same state %s\n", caller, line
...
void dbg_rtw_p2p_set_role(struct wifidirect_info *wdinfo, enum P2P_ROLE role, const char *caller, int line)
{
	if(wdinfo->role != role) {
		...
		DBG_8723A("[CONFIG_DBG_P2P]%s:%d set_role from %s to %s\n", caller, line
			, p2p_role_str[old_role], p2p_role_str[wdinfo->role]
		);
	} else {
		DBG_8723A("[CONFIG_DBG_P2P]%s:%d set_role to same role %s\n", caller, line
