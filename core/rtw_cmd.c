...
int rtw_cmd_filter(struct cmd_priv *pcmdpriv, struct cmd_obj *cmd_obj)
{
	...
	)
	{
		if(cmd_obj->cmdcode == GEN_CMD_CODE(_Set_Drv_Extra) )
		{
			...
			{
				//DBG_8723A("==>enqueue POWER_SAVING_CTRL_WK_CID\n");
				bAllow = _TRUE;
			}
		}
	}
	...
	{
		//DBG_8723A("%s:%s: drop cmdcode:%u, hw_init_completed:%u, cmdthd_running:%u\n", caller_func, __FUNCTION__,
		//	cmd_obj->cmdcode,
...
post_process:
	...
	do{
		...
		//DBG_8723A("%s: leaving... drop cmdcode:%u\n", __FUNCTION__, pcmd->cmdcode);
...
_func_enter_;
	...
	//DBG_8723A("rtw_addbareq_cmd, tid=%d\n", tid);
...
_func_enter_;
	...
	switch(lps_ctrl_type)
	{
		...
		case LPS_CTRL_JOINBSS:
			//DBG_8723A("LPS_CTRL_JOINBSS \n");
			LPS_Leave(padapter);
			break;
		case LPS_CTRL_CONNECT:
			//DBG_8723A("LPS_CTRL_CONNECT \n");
			mstatus = 1;//connect
			// Reset LPS Setting
		...
		case LPS_CTRL_SPECIAL_PACKET:
			//DBG_8723A("LPS_CTRL_SPECIAL_PACKET \n");
