...
int rtw_cmd_filter(struct cmd_priv *pcmdpriv, struct cmd_obj *cmd_obj)
{
	...
	if ((pcmdpriv->padapter->hw_init_completed == false && bAllow == false) ||
	    ((pcmdpriv->cmdthd_running == false	/* com_thread not running */
			...
			) && (pcmdpriv->padapter->isprimary == true
#endif
		)))
		return _FAIL;
	return _SUCCESS;
...
_func_enter_;
	...
	memset(psurveyPara->ssid, 0, sizeof(NDIS_802_11_SSID) * RTW_SSID_SCAN_AMOUNT);

	if (pssid) {
		int i;
		for (i = 0; i < ssid_max_num && i < RTW_SSID_SCAN_AMOUNT; i++) {
			if (pssid[i].SsidLength)
				memcpy(&psurveyPara->ssid[i], &pssid[i], sizeof(NDIS_802_11_SSID));
		}
	}
