...
bool Efuse_Read1ByteFromFakeContent(PADAPTER pAdapter, u16 Offset, u8 *Value)
{
...
bool Efuse_Write1ByteToFakeContent(PADAPTER pAdapter, u16 Offset, u8 Value)
{
	if (Offset >= EFUSE_MAX_HW_SIZE)
		return false;
	if (fakeEfuseBank == 0)
	...
	IN bool pseudotest)
{
	...
	ret = pAdapter->HalFunc.EfuseGetCurrentSize(pAdapter, efuseType, pseudotest);
...
{
	Adapter->HalFunc.ReadEFuse(Adapter, efuseType, _offset, _size_byte, pbuf, pseudotest);
}

void EFUSE_GetEfuseDefinition(PADAPTER pAdapter, u8 efuseType, u8 type, void *pOut, bool pseudotest)
{
	pAdapter->HalFunc.EFUSEGetEfuseDefinition(pAdapter, efuseType, type, pOut, pseudotest);
}
...
u8 efuse_OneByteWrite(PADAPTER pAdapter, u16 addr, u8 data, bool pseudotest)
{
	u8 tmpidx = 0;
	u8 result;
...
int Efuse_PgPacketRead(PADAPTER pAdapter, u8 offset, u8 *data, bool pseudotest)
{
	int	ret = 0;

	ret =  pAdapter->HalFunc.Efuse_PgPacketRead(pAdapter, offset, data, pseudotest);
					...
					bool pseudotest)
{
	...
	ret =  pAdapter->HalFunc.Efuse_PgPacketWrite(pAdapter, offset, word_en, data, pseudotest);
...
int
Efuse_PgPacketWrite_BT(PADAPTER pAdapter, u8 offset, u8 word_en, u8 *data, bool pseudotest)
{
	int ret;

	ret =  pAdapter->HalFunc.Efuse_PgPacketWrite_BT(pAdapter, offset, word_en, data, pseudotest);
...
u8 Efuse_WordEnableDataWrite(PADAPTER pAdapter, u16 efuse_addr, u8 word_en,
			     u8 *data, bool pseudotest)
{
	u8 ret = 0;
 ...
 *
 *---------------------------------------------------------------------------*/
void Efuse_ReadAllMap(PADAPTER pAdapter, u8 efuseType, IN u8 *Efuse, bool pseudotest)
{
	u16 mapLen = 0;

	Efuse_PowerSwitch(pAdapter, false, true);

	EFUSE_GetEfuseDefinition(pAdapter, efuseType, TYPE_EFUSE_MAP_LEN, (void *)&mapLen, pseudotest);

	efuse_ReadEFuse(pAdapter, efuseType, 0, mapLen, Efuse, pseudotest);
 ...
 *
 *---------------------------------------------------------------------------*/
static void efuse_ShadowRead1Byte(PADAPTER pAdapter, u16 Offset, IN u8 *Value)
{
...
/* Read Two Bytes */
static void efuse_ShadowRead2Byte(PADAPTER pAdapter, u16 Offset, IN u16 *Value)
{
	EEPROM_EFUSE_PRIV *pEEPROM = GET_EEPROM_EFUSE_PRIV(pAdapter);
	...
	IN bool pseudotest)
{
	...
	if (pEEPROM->bautoload_fail_flag == true) {
		...
		Efuse_ReadAllMap(pAdapter, efuseType, pEEPROM->efuse_eeprom_data, pseudotest);
...
void Efuse_InitSomeVar(PADAPTER pAdapter)
{
	u8 i;
