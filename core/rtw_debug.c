		...
		unsigned long count, void *data)
{
	...
	if (count < 3)
	{
		DBG_8723A("argument size is less than 3\n");
		return -EFAULT;
	}
	...
	if (buffer && !copy_from_user(tmp, buffer, sizeof(tmp))) {
		...
		if (num !=  3) {
			DBG_8723A("invalid write_reg parameter!\n");
			return count;
		}
		...
		switch(len)
		{
			...
			case 4:
				...
				break;
			default:
				DBG_8723A("error write length=%d", len);
				break;
		}
		...
		unsigned long count, void *data)
{
	...
	if (count < 2)
	{
		DBG_8723A("argument size is less than 2\n");
		return -EFAULT;
	}
	...
	if (buffer && !copy_from_user(tmp, buffer, sizeof(tmp))) {
		...
		if (num !=  2) {
			DBG_8723A("invalid read_reg parameter!\n");
			return count;
		}
		...
		unsigned long count, void *data)
{
	...
	if (count < 1)
	{
		DBG_8723A("argument size is less than 1\n");
		return -EFAULT;
	}
	...
	if (buffer && !copy_from_user(tmp, buffer, sizeof(tmp))) {
		...
		if (num !=  1) {
			DBG_8723A("invalid set_rssi_disp parameter!\n");
			return count;
		}

		if(enable)
		{
			DBG_8723A("Turn On Rx RSSI Display Function\n");
			padapter->bRxRSSIDisplay = enable ;
		}
		else
		{
			DBG_8723A("Turn Off Rx RSSI Display Function\n");
