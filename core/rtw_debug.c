...
int proc_get_read_reg(char *page, char **start, off_t offset, int count,
		      int *eof, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	...
	case 1:
		len += snprintf(page + len, count - len, "rtw_read8(0x%x) = 0x%x\n",
				proc_get_read_addr, rtw_read8(padapter,
				proc_get_read_addr));
		break;
	case 2:
		len += snprintf(page + len, count - len, "rtw_read16(0x%x) = 0x%x\n",
				proc_get_read_addr, rtw_read16(padapter,
				proc_get_read_addr));
		break;
	case 4:
		len += snprintf(page + len, count - len, "rtw_read32(0x%x) = 0x%x\n",
				proc_get_read_addr, rtw_read32(padapter,
				proc_get_read_addr));
		break;
	default:
		       ...
		       int *eof, void *data)
{
	...
	len += snprintf(page + len, count - len, "rssi:%d\nrxpwdb:%d\nsignal_strength:%u\nsignal_qual:%u\nnoise:%u\n",
			padapter->recvpriv.rssi, padapter->recvpriv.rxpwdb,
		...
		unsigned long count, void *data)
{
	...
	if (buffer && !copy_from_user(tmp, buffer, sizeof(tmp))) {
		...
		if (pregpriv && (mode == 0 || mode == 1 || mode == 2 ||
				 mode == 3)) {
