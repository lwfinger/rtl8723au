...
_func_enter_;
	...
	ibss = (s_cap & WLAN_CAPABILITY_IBSS) ==
               (d_cap & WLAN_CAPABILITY_IBSS);
        bss = (s_cap & WLAN_CAPABILITY_BSS) ==
...
_func_enter_;
	...
	if (mac_id > 0) {
		u16 media_status;
		media_status = (mac_id<<8); /*   MACID|OPMODE:0 means disconnect */
...
#define RTW_SCAN_RESULT_EXPIRE (IEEE80211_SCAN_RESULT_EXPIRE/HZ*1000-1000)
#else
 ...
 * The caller must hold the spinlock pmlmepriv->lock
*/
...
_func_enter_;
	...
	DBG_871X("==> rtw_set_key algorithm(%x), keyid(%x), key_mask(%x)\n",
		 psetkeyparm->algorithm,
		 psetkeyparm->keyid, pmlmepriv->key_mask);
	RT_TRACE(_module_rtl871x_mlme_c_, _drv_err_,
